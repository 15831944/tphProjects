...
public:
    ...
    int f4b_long(char *fieldname, long data);

    int f4b_int(char *fieldname,int data);

    int f4b_str(char *fieldname,char *fvalue);

    int f4b_date(char *fieldname, char *fvalue);
    ...
    //加载所有记录到内存
    int DbfLoadAllBuffer();

    //所有记录的缓冲区 
    char *m_DbfAllBuffer;

    //释放缓冲区
    int DbfReleaseAllBuffer();
    ...
    //到记录缓冲区的下一条记录
    void d4MemoryGetBuffer(int nRecNo,char *buf);
