...
{
	//qinj added 20140506
	if(m_DbfAllBuffer != NULL) delete []m_DbfAllBuffer;
...

{
...

   {
	  ...
	  //printf("Cannot open the DBF file[%s], or the file doesn't exist\n", dname);
...

   {
      ...
      //qinj added 20140606
	  if(!(version == 0x03 || version == 0x30 ))//不是dbf头
	  {
         d4close();
...
int CDBF::f4b_long(char *fieldname, long data)
...
int CDBF::f4b_date(char *fieldname, char *fvalue)

{
	
	char buf[8];
	
	sprintf(buf,"%08d", fvalue);
...

{
   ...
   if(write(handle,buf,records*record_len) != records*record_len)
   {
       return(0); 
   }
...
//加载所有记录到内存
int CDBF::DbfLoadAllBuffer()
{
	int nRecCount = d4reccount();
	if(nRecCount == 0 ) return -1;
	int nBufferLen = nRecCount * record_len;
	

	if(m_DbfAllBuffer != NULL ) 
	{
		delete []m_DbfAllBuffer;
		m_DbfAllBuffer = NULL;
	}

	m_DbfAllBuffer = new char[nBufferLen];
	lseek(handle,dbf_rec_pos(1),SEEK_SET);

	int rlen = read(handle,m_DbfAllBuffer,nBufferLen);
...
//到记录缓冲区的首记录
void CDBF::d4MemoryTop()
{
	return ;
}

//到记录缓冲区的下一条记录
void CDBF::d4MemoryNext()
{
	return ;
}

//获得一条记录的缓冲区
void CDBF::d4MemoryGetBuffer(int nRecNo,char *buf)
{
	memcpy(buf,m_DbfAllBuffer+nRecNo*record_len,record_len);
}
