...
void CCoolTree::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
    *pResult = 0;
}
...
BEGIN_MESSAGE_MAP(CTreeComboBox, CARCTipComboBox)
    ON_CONTROL_REFLECT(CBN_KILLFOCUS, CTreeComboBox::OnKillFocus)
END_MESSAGE_MAP()
void CTreeComboBox::OnKillFocus() 
{
    ShowWindow(SW_HIDE);
...
void CTreeComboBox::OnSelChange(NMHDR* pNMHDR, LRESULT* pResult)
{
    ShowWindow(SW_HIDE);
    m_nIndexSeled=GetCurSel();
    CString strValue;
...
DWORD_PTR CTreeComboBox::GetItemData(int nIndex)
{
