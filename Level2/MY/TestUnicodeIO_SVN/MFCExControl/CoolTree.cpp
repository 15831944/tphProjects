...
void CCoolTree::OnPaint() 
{
    ...
    {
        CRect rect;
        // Do not draw selected items or drop highlighted items
        ...
        if(pCNI && !(itemState & TVIS_SELECTED) && !(itemState & TVIS_DROPHILITED))
        {
            ...
            memDC.SetTextColor(/*pCNI->clrItem*/RGB(255, 0, 0));

            CString sItem = GetItemText(hItem);
            GetItemRect(hItem, &rect, TRUE);
            memDC.SetBkColor(/*GetSysColor(COLOR_WINDOW)*/RGB(255, 208, 168));
            switch(pCNI->net)
            {
            case NET_EDIT_INT:
            case NET_EDIT_FLOAT:
