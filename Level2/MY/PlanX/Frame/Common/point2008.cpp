...
CPoint2008 CPoint2008::getRandPoint (double radius) const
{
	...
	return CPoint2008 ((minX + (double) (xProb * interval)), (minY +
...
CPoint2008 CPoint2008::calcMidPoint (const CPoint2008& p, double speed,
...
double CPoint2008::operator * ( const CPoint2008& _pt ) const
{
...
void CPoint2008::setPoint (double X, double Y, double Z) 
{
	x = X;
	y = Y;
	z = Z;
...
void CPoint2008::DoOffset(double _xOffset, double _yOffset, double _zOffset )
{
	x += _xOffset;
	y += _yOffset;
	z += _zOffset;
...
void CPoint2008::offsetCoords( CPoint2008 ptFrom, double dDir, double dDist )
{
...
double CPoint2008::getDistanceTo(const  CPoint2008  & pointTo){
	double res;
	res=(x-pointTo.getX())*(x-pointTo.getX())+(y-pointTo.getY())*(y-pointTo.getY())+(z-pointTo.getZ())*(z-pointTo.getZ());
	return sqrt(res);
}

double CPoint2008::dotProduct( const CPoint2008& pt ) const
{
...
CPoint2008 CPoint2008::getLengtDir( double dlen ) const
{
	CPoint2008 ret = *this;
	ret.length(dlen);
	return ret;
