#include "StdAfx.h"
#include ".\billboardlinkedprocincrementalvisitreport.h"
#include "Reports.h"
#include "..\common\termfile.h"
#include "..\common\states.h"
#include "engine\terminal.h"

CBillboardLinkedProcIncrementalVisitReport::CBillboardLinkedProcIncrementalVisitReport(Terminal* _pTerm, const CString& _csProjPath)
:CBaseReport(_pTerm, _csProjPath)
{
}

CBillboardLinkedProcIncrementalVisitReport::~CBillboardLinkedProcIncrementalVisitReport(void)
{
}
void CBillboardLinkedProcIncrementalVisitReport::InitParameter( const CReportParameter* _pPara )
{
	CBaseReport::InitParameter( _pPara );
	m_usesInterval = 1;
}

void CBillboardLinkedProcIncrementalVisitReport::GenerateSummary( ArctermFile& p_file )
{
	...
	CProgressBar bar( "Generating Billboard Linked Proc Incremental Visit Report", 100, m_procIDlist.getCount(), TRUE );
...
void CBillboardLinkedProcIncrementalVisitReport::GenerateDetailed( ArctermFile& p_file )
{
	...
	if( nProcCount > 0 )
	{
		CProgressBar bar( "Generating Billboard Linked Proc Incremental Visit Report", 100, nProcCount, TRUE );
...
void CBillboardLinkedProcIncrementalVisitReport::getAverageThroughput (const ProcessorID *p_id)
{
	char str[MAX_PROC_ID_LEN];
	ProcessorID id;
	ProcLogEntry logEntry;
...
void CBillboardLinkedProcIncrementalVisitReport::writeAverage (const ProcessorID *id,
													  ArctermFile& p_file) const
{
	...
	p_file.writeFloat (  (m_lTotalPaxServed -m_lPassengersServed)/ hours, 2);
...
void CBillboardLinkedProcIncrementalVisitReport::writeEntry (const char *p_id, ArctermFile& p_file) const
{
	float hours = (float)(m_endTime - m_startTime).asHours();
...
void CBillboardLinkedProcIncrementalVisitReport::calculateThroughput( ProcessorID* _pProcID, ArctermFile& p_file )
{
	...
	for( int i=0 ; i< nResultItemCount ; ++i )
	{
		...
		p_file.writeInt (tempItem.m_lTotalThroughput - tempItem.m_lExposureCount);//normal count
		p_file.writeInt(tempItem.m_lTotalThroughput);//total
		p_file.writeInt(tempItem.m_lExposureCount);//incremental
		p_file.writeLine();				
	}	
}
...
void CBillboardLinkedProcIncrementalVisitReport::GetResult( CBillboardThroughputResult& _result,  ProcessorID* _pProcID )
{
		...
		if( _pProcID->idFits( procID ) )
		{
			...
			for (long i = 0; i < eventCount; i++)
			{
				...
				if (event.time >= (long)m_startTime && event.time <= (long)m_endTime)
					if (event.type == CompleteServiceBillboardLinkedProc )
					{
						_result.IncreaseCount( (ElapsedTime)( event.time / TimePrecision ),1 );
					}
					else if (event.type ==CompleteService )//total
					{
...
void CBillboardLinkedProcIncrementalVisitReport::calculateThroughput (ProcLogEntry& logEntry)
{
	m_nCount++;
