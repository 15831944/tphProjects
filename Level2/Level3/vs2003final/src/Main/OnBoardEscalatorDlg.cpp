...
IMPLEMENT_DYNAMIC(COnBoardEscalatorDlg, COnBoardObjectBaseDlg)

COnBoardEscalatorDlg::COnBoardEscalatorDlg(COnBoardEscalator *pEscalator,CAircaftLayOut *pLayout, int nProjID, CWnd* pParent /*= NULL*/)
:COnBoardObjectBaseDlg(pEscalator,pParent)
{
	m_nProjID = nProjID;
	m_pLayout = pLayout;
}

COnBoardEscalatorDlg::~COnBoardEscalatorDlg( void )
{

}
void COnBoardEscalatorDlg::DoDataExchange(CDataExchange* pDX)
{
	COnBoardObjectBaseDlg::DoDataExchange(pDX);
}
BEGIN_MESSAGE_MAP(COnBoardEscalatorDlg,COnBoardObjectBaseDlg)

END_MESSAGE_MAP()

BOOL COnBoardEscalatorDlg::OnInitDialog()
{
...
void COnBoardEscalatorDlg::OnDoubleClickPropTree(HTREEITEM hTreeItem)
{
	if(hTreeItem == m_hItemWidth)
	{
		m_hRClickItem = hTreeItem;
		double dValue = 0.0;
		dValue = ((COnBoardEscalator *)m_pObject)->GetWidth();
...
void COnBoardEscalatorDlg::OnContextMenu(CMenu& menuPopup, CMenu *& pMenu)
{
...
LRESULT COnBoardEscalatorDlg::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{		
...
FallbackReason COnBoardEscalatorDlg::GetFallBackReason()
{
	return PICK_TWOPOINTS;
}
void COnBoardEscalatorDlg::SetObjectPath(const ARCPath3& path)
{
	if (m_hItemServiceLocation == m_hRClickItem)
	{
		...
		((COnBoardEscalator *)m_pObject)->SetPathService(floorPath);
...
void COnBoardEscalatorDlg::LoadTree()
{
	m_treeProp.DeleteAllItems();

	COnBoardEscalator *pEscalator = (COnBoardEscalator *)m_pObject;
	...
	InsertPathToTree(pEscalator->m_sevicePath,m_hItemServiceLocation);	
	...
	{	
		...
		//width
		{
			double dWidth = pEscalator->GetWidth();
			CString csLabel;
...
void COnBoardEscalatorDlg::OnOK()
{
	...
	try
	{
		((COnBoardEscalator *)m_pObject)->SetLayOut(m_pLayout);
		((COnBoardEscalator *)m_pObject)->SaveData(m_pLayout->GetID());
...
bool COnBoardEscalatorDlg::UpdateOtherData()
{	
	return true;
}

bool COnBoardEscalatorDlg::ConvertUnitFromDBtoGUI(void)
{
	//it do nothing , so return (false);
	return (false);
}

bool COnBoardEscalatorDlg::RefreshGUI(void)
{
	LoadTree();
	return (true);
}

bool COnBoardEscalatorDlg::ConvertUnitFromGUItoDB(void)
{
	//it do nothing , so return (false);
	return (false);
}

void COnBoardEscalatorDlg::InsertPathToTree( const Path& path, HTREEITEM hParentItem )
{
...
void COnBoardEscalatorDlg::InsertPointToTree( const Point& point, HTREEITEM hParentItem,const CString& strDeckName )
{
	AirsideObjectTreeCtrlItemDataEx aoidDataEx;
	ItemStringSectionColor isscStringColor;
	CString strTemp = _T("");
...
void COnBoardEscalatorDlg::DefineZPos()
{
	if(m_pLayout == NULL)
		return;
	...
	const Path& pPathService = ((COnBoardEscalator *)m_pObject)->GetPathService();
