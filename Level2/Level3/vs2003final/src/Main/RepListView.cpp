...
void CRepListView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_GridControl(pDX, IDC_REPORT_GRIDCTRL,m_wndARCGrid);
}
...
void CRepListView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
	if(IsWindow(m_hWnd))
	{
		...
		if(GetReportParameter()->IsMultiRunReport())
		{
			...
			m_pRepListViewBaseOperator = new CRepListViewMultiRunReportOperator(&m_wndARCGrid,&(pDoc->GetARCReportManager()),this);
		}
		else
		{
			m_pRepListViewBaseOperator = new CRepListViewSingleReportOperator(&m_wndARCGrid,&(pDoc->GetARCReportManager()),this);
		}
...
//	m_wndARCGrid.DeleteAllItems();
//	for( int i=0; i<m_nColumnCount; i++ )
//	{
//		m_wndARCGrid.DeleteColumn( 0 );
//	}
//	
...
//	{
//		m_wndARCGrid.InsertColumn( i, csColumnLabel[i], nFormat[i], nDefaultColumnWidth[i] );
...
//		int nItemIdx = m_wndARCGrid.InsertItem( nDBIdx, str );
//		int i;	
//        
//		//********************************************************
...
//					m_wndARCGrid.SetItemText( nItemIdx, i,  ColStr );
//				}
//				else
//					m_wndARCGrid.SetItemText( nItemIdx, i,  str );
//			}
//			float fSpeed;
//			csvFile.getFloat( fSpeed );
//			fSpeed = static_cast<float>(UNITSMANAGER->ConvertLength( (double)fSpeed ));
//			sprintf( str, "%.2f", fSpeed );
//			m_wndARCGrid.SetItemText( nItemIdx, 6,  str );
//			csvFile.getField( str, 128 );
//			m_wndARCGrid.SetItemText( nItemIdx, 7,  str );
//
//
//		}
...
//				m_wndARCGrid.SetItemText( nItemIdx,1,str );
//				csvFile.getFloat( dLength );
//				dLength = pUM->ConvertLength( dLength * SCALE_FACTOR );
//				sprintf( str,"%d",(int)dLength );
//				m_wndARCGrid.SetItemText( nItemIdx,2,str );
//				csvFile.getFloat( dLength );
//				dLength = pUM->ConvertLength( dLength * SCALE_FACTOR );
//				sprintf( str,"%d",(int)dLength );
//				m_wndARCGrid.SetItemText( nItemIdx,3,str );
//				csvFile.getField( str,128 );
//				m_wndARCGrid.SetItemText( nItemIdx,4,str );
//				
//				for( int i=5; i<=14; ++i )
//				{
//					csvFile.getFloat( dLength );
...
//		//****************************************************************
//
//		m_wndARCGrid.SetItemData( nItemIdx, nDBIdx );
//		nDBIdx++;
//    }
//	
//	m_wndARCGrid.SetRedraw( TRUE );
//	Invalidate();
//	UpdateWindow();
//	
...
//    }
//	CTermPlanDoc * pDoc=(CTermPlanDoc *)GetDocument();
//	if(m_wndARCGrid.GetItemCount()==0)
//	{	
...
void CRepListView::OnInitialUpdate() 
{
	...
	m_wndARCGrid.SetFixedRowCount(1);
	m_wndARCGrid.SetFixedColumnCount(1);
	m_wndARCGrid.SetEnableNoScroll(TRUE);
	m_wndARCGrid.AutoSize();
...
void CRepListView::OnSize(UINT nType, int cx, int cy)
{
	CFormView::OnSize(nType,cx,cy);
 	...
 	if (m_wndARCGrid.GetSafeHwnd())
 	{
 		m_wndARCGrid.MoveWindow(0,0,cx,cy);
 		m_wndARCGrid.FitSize();
 	}
}
...
void CRepListView::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
 ...
 //   case CDDS_ITEMPREPAINT: // Requested notification
	//	int nDBIdx = m_wndARCGrid.GetItemData(lplvcd->nmcd.dwItemSpec);
	//	
...
// 	if (m_wndARCGrid.GetSelectedCount() == 0)						// if there are no selected items in the list control
...
// 		dc.AbortDoc();
// 	dc.Detach();
	m_wndARCGrid.PrintList(m_sHeader,m_sFoot);
}
...
#define HFONTNAME "MS Sans Serif"
    ...
    int nCol = 0;/* = m_wndARCGrid.GetHeaderCtrl()->GetItemCount();*/
	HDITEM hi;
	hi.mask = HDI_TEXT ;
	hi.cchTextMax = 19;
	char chBuffer[20];
	...
	//	m_wndARCGrid.GetHeaderCtrl()->GetItem( i,&hi);
	...
	int nIndex=1;
	int nRowCount = 0/* = m_wndARCGrid.GetItemCount()*/;
	while ( nIndex <= nRowCount ) 
	{
		...
		for( int nSubitem = 0; nSubitem < nCol; nSubitem++ )
		{
			...
			char chLine[20];
		//	m_wndARCGrid.GetItemText(nIndex-1,nSubitem,chLine,19);
...
#define HFONTNAME "MS Sans Serif"
	...
	//Format the heading
	// get listctrl's header info
    int nCol = 1 /*= m_wndARCGrid.GetHeaderCtrl()->GetItemCount()*/;
	HDITEM hi;
	hi.mask = HDI_TEXT ;
	hi.cchTextMax = 19;
	char chBuffer[20];
	...
	//Scroll through the list
	int nIndex=1;
	POSITION ps  = NULL/*= m_wndARCGrid.GetFirstSelectedItemPosition()*/;
	if(ps == NULL) 
		return;
//	int curSelectedItem = m_wndARCGrid.GetNextSelectedItem(ps);
	int nRowCount = m_wndARCGrid.GetSelectedCount();
	while ( nIndex <= nRowCount ) 
	{
		if (pInfo && y > nPageHeight-nPageVMargin-nFooterHeight-nLineHeight-6) 
		{   // start a new page
		...
		for( int nSubitem = 0; nSubitem < nCol; nSubitem++ )
		{
			...
			char chLine[20];
//			m_wndARCGrid.GetItemText(curSelectedItem, nSubitem, chLine, 19);
			pDC->TextOut(nPageHMargin+nSubitem*nSpace, y,chLine);
		}
//		curSelectedItem = m_wndARCGrid.GetNextSelectedItem(ps);
		y += nLineHeight; //Adjust y position
		nIndex++; 
	}
	if (pInfo) {
...
//	m_wndARCGrid.InsertColumn(0, "Time", LVCFMT_CENTER, 60);
...
//	{
//		type = dtSTRING;
//		m_wndARCGrid.InsertColumn( i+1, (CString)result.m_vstrProcess[i].c_str(), LVCFMT_CENTER, nWidth );
...
//	for( i = 0; i < nRow; i++ )
//	{
//		int nItemIdx = m_wndARCGrid.InsertItem( i, GetAbsDateTime(vRecord[i].time.printTime()));//test
//
...
//			m_wndARCGrid.SetItemText( nItemIdx, j+1, strProcResult );
//		}
//	}	
//
//	m_wndARCGrid.SetRedraw( TRUE );
