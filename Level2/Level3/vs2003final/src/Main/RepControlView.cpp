...
void CRepControlView::OnButtonCancel() 
{
	...
	*/
	...
	else
	{
		for( i=0; i<iSize-1; ++i )
		{
			m_strProcessor += vProcGroup[i].GetIDString();
			m_strProcessor += ";";
		}
		m_strProcessor += vProcGroup[i].GetIDString();
	}	
...
void CRepControlView::SaveReport()
{
	...
	CFileDialog savedlg(FALSE,strExten,NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,strFilter,this );
	if( savedlg.DoModal() == IDOK )
	{
		CString strFileName = savedlg.GetPathName();
...
void CRepControlView::LoadReport()
{
	CString strExten	= getExtensionString();
	CString strFilter	= "Report File(*." + strExten + ")|*." + strExten + "|All Type(*.*)|*.*||";
	CFileDialog loaddlg(TRUE,NULL,NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, strFilter,this );
