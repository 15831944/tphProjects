...
void CNodeView::OnLayoutexport() 
{
	...
	CFileDialog dlgFile( FALSE, ".zip","LayoutExport.zip",OFN_OVERWRITEPROMPT|OFN_HIDEREADONLY,"zip Files (*.zip)|*.zip||" );
...
void CNodeView::OnLayoutimportAppend() 
{
	...
	CFileDialog dlgFile( TRUE, ".zip","*.zip",OFN_OVERWRITEPROMPT|OFN_HIDEREADONLY,"zip Files (*.zip)|*.zip||" );
	CString sZipFileName;
	if (dlgFile.DoModal() == IDOK)
		sZipFileName = dlgFile.GetPathName();
	else return;
	...
	if(MessageBox(msg.GetBuffer(0),NULL,MB_OKCANCEL)==IDOK)
	{
		...
		for( i = 0; i < static_cast<int>(pNewPlacement->m_vUndefined.size()); i++ )
		{
			CProcessor2* pProc2 = pNewPlacement->m_vUndefined[i];
			pDoc->GetCurrentPlacement()->m_vUndefined.push_back(pProc2);
		}
		for( i = 0; i < static_cast<int>(pNewPlacement->m_vDefined.size()); i++ )
		{
			CProcessor2* pProc2 = pNewPlacement->m_vDefined[i];
			iter=addGroup.begin();
			for(;iter!=addGroup.end();iter++)
...
void CNodeView::OnLayoutimportReplace() 
{
	...
	for(i=0;i<pDoc->GetTerminal().procList->getProcessorCount();i++)
	{
...
void CNodeView::SwapTwoFloor(int iSelFloor, int iTargetFloor)
{
	...
	double selFloorVisualHeight = pSelFloor->Altitude();
	double selFloorRealHeight = pSelFloor->RealAltitude();
	double tarFloorVisualHeight = pTargetFloor->Altitude();
	double tarFloorRealHeight = pTargetFloor->RealAltitude();

	pSelFloor->Altitude(tarFloorVisualHeight);
	pSelFloor->RealAltitude(tarFloorRealHeight);
	pTargetFloor->Altitude(selFloorVisualHeight);
	pTargetFloor->RealAltitude(selFloorRealHeight);
