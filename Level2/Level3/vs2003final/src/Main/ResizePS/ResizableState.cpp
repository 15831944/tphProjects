...
BOOL CResizableState::SaveWindowRect(LPCTSTR pszSection, BOOL bRectOnly)
{
	...
	//	return  FALSE;
	//CINIFileHandle handle ;
	//if(!handle.LoadFile(regReader.m_Path.m_AppPath))
	//	return FALSE;
	//handle.SetCurrentGroup(pszSection) ;
 //   handle.SetData(PLACEMENT_ENT,data) ;
	//return handle.SaveData(regReader.m_Path.m_AppPath) ;
	return ((CTermPlanApp*)AfxGetApp())->WriteProfileString(pszSection, PLACEMENT_ENT, data);
}
...
BOOL CResizableState::LoadWindowRect(LPCTSTR pszSection, BOOL bRectOnly)
{
	...
	data = ((CTermPlanApp*)AfxGetApp())->GetProfileString(pszSection, PLACEMENT_ENT);
	/*CReadRegister regReader ;
	if(!regReader.ReadRegisterPath(CString()))
		return  FALSE;
	CINIFileHandle handle ;
	if(!handle.LoadFile(regReader.m_Path.m_AppPath))
		return FALSE;
	handle.SetCurrentGroup(pszSection) ;
	handle.GetData(PLACEMENT_ENT,data) ;
	handle.SaveData(regReader.m_Path.m_AppPath) ;*/
