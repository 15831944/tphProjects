...
CBmpBtnDropListEX::CBmpBtnDropListEX()
{
	m_nShapeSetIndex = -1;
}
...
BEGIN_MESSAGE_MAP(CBmpBtnDropListEX, CBitmapButton)
	//{{AFX_MSG_MAP(CBmpBtnDropList)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CBmpBtnDropListEX::DisplayListWnd()
{
...
BOOL CBmpBtnDropListEX::Create(CRect rect, UINT nID, CWnd *pParent)
{
...
void CBmpBtnDropListEX::SetShapeSetIndex(int nShapeSetIndex)
{
	m_nShapeSetIndex = nShapeSetIndex;
}

void CBmpBtnDropListEX::CalculateDroppedRect(LPRECT lpDroppedRect)
{
...
int CBmpBtnDropListEX::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CBmpBtnDropListEX::HideCtrl()
{
	ShowWindow(SW_HIDE);
}

BOOL CBmpBtnDropListEX::CreateListWnd()
{
