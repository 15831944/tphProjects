...
CAircraftDispPropDlg::CAircraftDispPropDlg(CWnd* pParent)
	: CDialog(CAircraftDispPropDlg::IDD, pParent)
{
	m_nSelectedPDPSet = -1;
	m_bNewPDPSet = FALSE;
}

void CAircraftDispPropDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CAircraftDispPropDlg, CDialog)
...
/////////////////////////////////////////////////////////////////////////////
// CAircraftDispPropDlg message handlers

BOOL CAircraftDispPropDlg::OnInitDialog() 
{
	...
	// Create column
	m_listctrlProp.InsertColumn( 0, "Flight Type", LVCFMT_CENTER, 100 );
	m_listctrlProp.InsertColumn( 1, "Color", LVCFMT_CENTER, 100 );
	m_listctrlProp.InsertColumn( 2, "Logo On/Off", LVCFMT_CENTER, 100 );
	m_listctrlProp.InsertColumn( 3, "Line Type", LVCFMT_CENTER, 150 );	
	m_listctrlProp.InsertColumn( 4, "Visible", LVCFMT_CENTER, 50 );
	//m_listctrlProp.InsertColumn( 4, "Leave Trail", LVCFMT_CENTER, 100 );
	m_listctrlProp.InsertColumn( 5, "Tracer Density", LVCFMT_CENTER, 90 );
	m_listctrlProp.InsertColumn( 6, "Scale Size", LVCFMT_CENTER, 80 );
...
void CAircraftDispPropDlg::OnButtonSave()
{
	CTermPlanDoc* pDoc = GetDocument();
	pDoc->m_aircraftDispProps.saveInputs(pDoc->m_ProjInfo.path, true);
}

void CAircraftDispPropDlg::OnCancel() 
{
	CDialog::OnCancel();

	CTermPlanDoc* pDoc = GetDocument();
	pDoc->m_aircraftDispProps.loadInputs(pDoc->m_ProjInfo.path, &(pDoc->GetTerminal()));
	UpdateAircraftDispPropIn3D();
}

void CAircraftDispPropDlg::OnOK() 
{
	CTermPlanDoc* pDoc = GetDocument();
	pDoc->m_aircraftDispProps.saveInputs(pDoc->m_ProjInfo.path, true);
...
void CAircraftDispPropDlg::InitToolbar()
{
	CRect rc;
...
int CAircraftDispPropDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CAircraftDispPropDlg::OnSize(UINT nType, int cx, int cy) 
{
...
void CAircraftDispPropDlg::OnToolbarbuttonadd() 
{
	...
	{
		CTermPlanDoc* pDoc	= GetDocument();	
		CAircraftDispPropItem*  pItem = new CAircraftDispPropItem( dlg.GetFlightType().GetName() );
		pItem->SetFlightType( dlg.GetFlightType() );
		pDoc->m_aircraftDispProps.AddDispPropItem( pItem );
		int nCount = m_listctrlProp.GetItemCount();
		m_listctrlProp.InsertACDispItem( nCount, pItem );
...
void CAircraftDispPropDlg::OnToolbarbuttondel() 
{
	...
	else
	{
		...
		CTermPlanDoc* pDoc	= GetDocument();	
		// delete from db
		pDoc->m_aircraftDispProps.DeleteItem( nItem-2 );
...
void CAircraftDispPropDlg::OnToolbarbuttonedit() 
{
	// TODO: Add your command handler code here

}

void CAircraftDispPropDlg::OnClickListDispprop(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
LRESULT CAircraftDispPropDlg::OnPaxProcEdit( WPARAM wParam, LPARAM lParam )
{
	int nItem = (int)wParam;
	if( nItem <2 )
		return 0;
	CAircraftFilterDlg dlg( m_pParentWnd );
	CAircraftDispPropItem* pOldItem = (CAircraftDispPropItem*)m_listctrlProp.GetItemData( (int)wParam );
	dlg.SetFlightType( *(pOldItem->GetFlightType()) );
	if( dlg.DoModal() == IDOK )
	{
		CTermPlanDoc* pDoc	= GetDocument();	
		CAircraftDispPropItem*  pNewItem = new CAircraftDispPropItem( *pOldItem );
		pNewItem->SetFlightType( dlg.GetFlightType() );					
		/*
		pDoc->m_aircraftDispProps.DeleteItem( nItem-2 );
		pDoc->m_aircraftDispProps.AddDispPropItem( pItem );
		*/
		pDoc->m_aircraftDispProps.DeleteAndReplaceItem( nItem-2, pNewItem );
		m_listctrlProp.DeleteItem( nItem );
		m_listctrlProp.InsertACDispItem( nItem, pNewItem );
...
void CAircraftDispPropDlg::OnClose() 
{
...
void CAircraftDispPropDlg::OnItemChangedListPDPSets(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	if(pnmv->uOldState & LVIS_SELECTED) {
		...
		CTermPlanDoc* pDoc = GetDocument();
		pDoc->m_aircraftDispProps.SetCurrentSet(m_nSelectedPDPSet);
	}
	else
		ASSERT(0);
...
void CAircraftDispPropDlg::OnPDPSetAdd()
{
	CTermPlanDoc* pDoc = GetDocument();

	int nPDPSetCount = pDoc->m_aircraftDispProps.GetSetCount();
	pDoc->m_aircraftDispProps.AddSet("undefined", new CAircraftDispPropItem("DEFAULT"), new CAircraftDispPropItem("OVERLAP"));
...
void CAircraftDispPropDlg::OnPDPSetDelete()
{
	...
	CTermPlanDoc* pDoc = GetDocument();
	ASSERT(m_nSelectedPDPSet >= 0 && m_nSelectedPDPSet < pDoc->m_aircraftDispProps.GetSetCount());
	pDoc->m_aircraftDispProps.DeletSet(m_nSelectedPDPSet);

	m_nSelectedPDPSet = pDoc->m_aircraftDispProps.GetCurrentSetIdx();
...
void CAircraftDispPropDlg::UpdatePDP()
{
	m_listctrlProp.DeleteAllItems();
	CTermPlanDoc* pDoc = GetDocument();
	CAircraftDispPropItem* pItem = pDoc->m_aircraftDispProps.GetDefaultDispProp();
	m_listctrlProp.InsertACDispItem( 0, pItem );
	pItem = pDoc->m_aircraftDispProps.GetOverLapDispProp();
	m_listctrlProp.InsertACDispItem( 1, pItem );
	int nCount = pDoc->m_aircraftDispProps.GetCount();
	for( int i=0; i<nCount; i++ )
	{
		pItem = pDoc->m_aircraftDispProps.GetDispPropItem( i );
		m_listctrlProp.InsertACDispItem( i + 2, pItem );
	}
}

void CAircraftDispPropDlg::OnEndLabelEditListPDPSets(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult = 0;
	...
	ASSERT(pDoc->m_aircraftDispProps.GetCurrentSetIdx() == nIdx);
	pDoc->m_aircraftDispProps.RenameCurrentSet(pDispInfo->item.pszText);
...
void CAircraftDispPropDlg::UpdatePDPSetList()
{
	CTermPlanDoc* pDoc	= GetDocument();
	...
	for(int i=0; i<nPDPSetCount; i++) {
		m_listNamedSets.InsertItem(LVIF_TEXT | LVIF_PARAM, i, pDoc->m_aircraftDispProps.GetSetName(i), 0, 0, 0, (LPARAM) i);
	}
	m_nSelectedPDPSet = pDoc->m_aircraftDispProps.GetCurrentSetIdx();
	m_listNamedSets.SetItemState(m_nSelectedPDPSet, LVIS_SELECTED, LVIS_SELECTED );
}

void CAircraftDispPropDlg::UpdateAircraftDispPropIn3D() const
{
	GetDocument()->UpdateAllViews(NULL, VM_RELOAD_FLIGHT_DISPLAY_PROPERTY);
}

CTermPlanDoc* CAircraftDispPropDlg::GetDocument() const
{
...
LRESULT CAircraftDispPropDlg::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	// TODO: Add your specialized code here and/or call the base class
	if (message == AC_DISP_PROP_CHANGE1)
	{
		UpdateAircraftDispPropIn3D();
	}
