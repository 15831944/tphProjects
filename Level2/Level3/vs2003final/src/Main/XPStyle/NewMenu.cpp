...
COLORREF BleachColor(int Add, COLORREF ColA)
{
  return RGB( min (GetRValue(ColA)+Add, 255),
              min (GetGValue(ColA)+Add, 255),
              min (GetBValue(ColA)+Add, 255));
}
...
public:
  ...
  public:
    CMenuHookData(HWND hWnd,BOOL bSpecialWnd) 
      : m_dwData(bSpecialWnd),m_bDrawBorder(FALSE),m_Point(0,0),m_hRgn((HRGN)1)
    {
      // Safe actual menu
...
void CNewMenu::SetMenuDrawMode(UINT mode)
{
  ...
  if(mode&1)
  {
   // TRACE(_T("Draw menu no border\n"));
  }
  else
  {
   // TRACE(_T("Draw menu with border\n"));
  }
#endif //  _TRACE_MENU_  
  ...
  else
  {
    ...
    if(state&ODS_SELECTED)
    {
      ...
      if(!bIsMenuBar)
      {
        ...
        if(bHighContrast)
        {
          pDC->FillSolidRect (rect2,GetSysColor(COLOR_HIGHLIGHT));
        }
        else
        {
          DrawGradient(pDC,rect2,LightenColor(30,GetSysColor(COLOR_HIGHLIGHT)),DarkenColor(30,GetSysColor(COLOR_HIGHLIGHT)),false,true);
        }
        pDC->Draw3dRect(rect2 ,GetSysColor(COLOR_3DSHADOW),GetSysColor(COLOR_3DHILIGHT));
      }
      else
...
#ifdef  _TRACE_MENU_
  HANDLE hLogFile;
   hLogFile = CreateFile(_T("c:\\log.txt"), GENERIC_WRITE, FILE_SHARE_WRITE, 
                                            ...
                                            LPARAM lParam)  // second message parameter
{
  ...
  case WM_NCPAINT:
    {
#ifdef _TRACE_MENU_   
      // TRACE(_T("WM_NCPAINT (0x%x) 0x%X, 0x%X\n"),hWnd,wParam,lParam);
#endif 
      CMenuHookData* pData = GetMenuHookData(hWnd);
      ASSERT(pData);
  ...
  case WM_ERASEBKGND:
    {
      ...
      {
        CMenuHookData* pData = GetMenuHookData(hWnd);
        if(pData && !(pData->m_dwData&8) && !pData->m_bDrawBorder )
        {
