...
CRepListViewMultiRunReportOperator::CRepListViewMultiRunReportOperator( ARCGridCtrl *pListCtrl, CARCReportManager* pARCReportManager,CRepListView *pListView )
...
{
	ARCGridCtrl& wndListCtrl = GetListCtrl();
	...
	for( int i=0; i<m_nColumnCount; i++ )
	{
		...
		GetListHeaderCtrl()->SetDataType( i+1, type[i] );
	}
	...
	{	
		...
		GetListCtrl().SetItemText(i+1,2,dataList[i][j].eMinimum.printTime());
		GetListCtrl().SetItemText(i+1,3,dataList[i][j].eAverage.printTime());
		GetListCtrl().SetItemText(i+1,4,dataList[i][j].eMaximum.printTime());

		GetListCtrl().SetItemText(i+1,5,dataList[i][j].eQ1.printTime());
		GetListCtrl().SetItemText(i+1,6,dataList[i][j].eQ2.printTime());
		GetListCtrl().SetItemText(i+1,7,dataList[i][j].eQ3.printTime());
		GetListCtrl().SetItemText(i+1,8,dataList[i][j].eP1.printTime());
		GetListCtrl().SetItemText(i+1,9,dataList[i][j].eP5.printTime());
		GetListCtrl().SetItemText(i+1,10,dataList[i][j].eP10.printTime());
		GetListCtrl().SetItemText(i+1,11,dataList[i][j].eP90.printTime());
		GetListCtrl().SetItemText(i+1,12,dataList[i][j].eP95.printTime());
		GetListCtrl().SetItemText(i+1,13,dataList[i][j].eP99.printTime());
		GetListCtrl().SetItemText(i+1,14,dataList[i][j].eSigma.printTime());
		GetListCtrl().SetItemText(i+1,15,dataList[i][j].strPaxType);

		CString strCount;
		strCount.Format(_T("%d"),dataList[i][j].nCount);
		GetListCtrl().SetItemText(i+1,16,strCount);


		GetListCtrl().SetItemData(i+1,i);
	}
	wndListCtrl.AutoSize();
...
void CRepListViewMultiRunReportOperator::SetSummaryCriticalQueueContents()
{
	...
	for (size_t i = 0; i < vSimResultFullName.size(); ++i)
	{
		for (size_t j =0; j < dataList[i].size(); ++j)
		{
			...
			GetListCtrl().SetItemText(nItemIndex+1,2,dataList[i][j].strProcName);
			CString strGroupSize;
			strGroupSize.Format(_T("%d"),dataList[i][j].nGroupSize);
			GetListCtrl().SetItemText(nItemIndex+1,3,strGroupSize);

			CString strAvgQueue;
			strAvgQueue.Format(_T("%.2f"),dataList[i][j].fAvgQueue);
			GetListCtrl().SetItemText(nItemIndex+1,4,strAvgQueue);

			CString strMaxQueue;
			strMaxQueue.Format(_T("%d"),dataList[i][j].nMaxQueue);
			GetListCtrl().SetItemText(nItemIndex+1,5,strMaxQueue);


			GetListCtrl().SetItemText(nItemIndex+1,6,dataList[i][j].eMaxQueueTime.printTime());

			GetListCtrl().SetItemData(nItemIndex+1,nItemIndex);
			nItemIndex+=1;
		}
...
//	ResetAllContent();
	...
	for (size_t i = 0; i < vSimResultFullName.size(); ++i)
	{
		...
		for (size_t n = 0; n < rangeCount; ++n)
		{
			...
			wndListCtrl.SetItemText(i+1, n + 2, strCount);
...
//	ResetAllContent();
	...
	for (MultiRunsReport::Summary::PaxDataList::iterator iter = dataList.begin(); iter != dataList.end(); ++iter)
	{
		...
		for (size_t i = 0; i < vSimResults.size(); ++i)
		{
			...
			wndListCtrl.SetItemText(nRow+1, 1 + i * 3 + 1, strValue);

			strValue.Format(_T("%.2f"), iter->second[i].fAveValue);
			wndListCtrl.SetItemText(nRow+1, 2 + i * 3 + 1, strValue);

			strValue.Format(_T("%.2f"), iter->second[i].fMaxValue);
			wndListCtrl.SetItemText(nRow+1, 3 + i * 3 + 1, strValue);
		}
