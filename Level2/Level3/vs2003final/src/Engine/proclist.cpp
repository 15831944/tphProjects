...
{
	if (!stricmp (procName, "START"))
		return startProcessor;
	else if (!stricmp (procName, "END"))
		return endProcessor;
	else if	(!stricmp (procName,"FROM-GATE"))
		return fromGateProcessor;
	else if (!stricmp (procName, "TO-GATE"))
		return toGateProcessor;
	else if( !stricmp ( procName,"BAGGAGE-DEVICE"))
		return baggageDeviceProcessor;
...
Processor *ProcessorList::readProcessor (ArctermFile& procFile, bool _bFirst /*=false*/ )
{
	...
	if (!stricmp (string, "DPND_SOURCE"))
		proc = new DependentSource;
	else if (!stricmp (string, "DPND_SINK"))
		proc = new DependentSink;
	else if (!stricmp (string, "GATE"))
		proc = new GateProcessor;
	else if (!stricmp (string, "LINE"))
		proc = new LineProcessor;
	else if (!stricmp(string, "RETAIL"))
		proc = new RetailProcessor;
	else if (!stricmp (string, "BAGGAGE"))
		proc = new BaggageProcessor;
	else if (!stricmp (string, "HOLD_AREA"))
		proc = new HoldingArea;
	else if (!stricmp (string, "BARRIER"))
		proc = new Barrier;
	else if (!stricmp (string, "FLOOR")) 
		proc = new FloorChangeProcessor;
	else if (!stricmp(string,"INTEGRATED_STATION"))
		proc = new IntegratedStation ;
	else if (!stricmp(string,"MOVING_SIDE_WALK"))
		proc = new MovingSideWalk();
	else if(!stricmp(string,"Elevator"))
		proc =new ElevatorProc;
	else if(!stricmp(string,"Conveyor"))
		proc =new Conveyor;
	else if(!stricmp(string,"Stair"))
		proc =new Stair;
	else if(!stricmp(string,"Escalator"))
		proc =new Escalator;
	else if(!stricmp(string,"BridgeConnector"))
		proc =new BridgeConnector;
	else if(!stricmp(string,"ARP"))
		proc = new ArpProc();
	else if(!stricmp(string,"RUNWAY"))
		proc = new RunwayProc();
	else if(!stricmp(string,"TAXIWAY"))
		proc = new TaxiwayProc();
	else if(!stricmp(string,"STAND"))
		proc = new StandProc();
	else if(!stricmp(string,"NODE"))
		proc = new NodeProc();
	else if(!stricmp(string,"DEICE_STATION"))
		proc = new DeiceBayProc();
	else if(!stricmp(string,"FIX"))
		proc = new FixProc();
	else if (!stricmp(string,"CONTOUR"))
		proc = new ContourProc;
	else if(!stricmp(string ,"APRON"))
		proc= new ApronProc;	
	else if (!stricmp(string,"Billboard"))
		proc = new BillboardProc;
	else if(!stricmp(string,"Hold"))
		proc = new HoldProc;
	else if(!stricmp(string,"Sector"))
		proc = new SectorProc;
	else if(!stricmp(string,StretchProc::m_strTypeName))
		proc  = new StretchProc();
	else if(!stricmp(string,IntersectionProc::m_strTypeName))
		proc = new IntersectionProc();
	else if( !stricmp(string,RoundAboutProc::m_strTypeName) )
		proc = new RoundAboutProc();
	else if( !stricmp(string,TurnOffProc::m_strTypeName) )
		proc = new TurnOffProc();
	else if( !stricmp(string,LaneAdapterProc::m_strTypeName) )
		proc = new LaneAdapterProc();
	else if( !stricmp(string,OverPassProc::m_strTypeName) )
		proc = new OverPassProc();
	else if( !stricmp(string,TrafficlightProc::m_strTypeName))
		proc = new TrafficlightProc();
	else if( !stricmp(string,YieldSignProc::m_strTypeName ))
		proc = new YieldSignProc;
	else if( !stricmp(string,StopSignProc::m_strTypeName))
		proc = new StopSignProc;
	else if( !stricmp(string,TollGateProc::m_strTypeName))
		proc = new TollGateProc;
	else if( !stricmp(string,LineParkingProc::m_strTypeName))
		proc = new LineParkingProc;
	else if( !stricmp(string,NoseInParkingProc::m_strTypeName))
...
void ProcessorList::AddAllDynamicCreatedProcIntoList()
{
	...
	if( iDynamicCreatedProcCount > 0 )
	{
		...
		for( i=0; i<iDynamicCreatedProcCount; ++i )
		{
