...
OnboardEscalatorInSim::OnboardEscalatorInSim(COnBoardEscalator *pEcalator,OnboardFlightInSim* pFlightInSim)
:OnboardDeviceInSim(pEcalator,pFlightInSim)
,m_pElementSpace(NULL)
,m_pBackElementSpace(NULL)
{
	m_realpath = pEcalator->GetLayOut()->GetDeckManager()->GetRealPath(pEcalator->GetPathService());
	CalculateSpace();
}

OnboardEscalatorInSim::~OnboardEscalatorInSim()
{
	ClearSpace();
}

GroundInSim* OnboardEscalatorInSim::GetGround(CDeck* pDeck)
{
	return m_pFlightInSim->GetGroundInSim(pDeck);
}

void OnboardEscalatorInSim::ClearSpace()
{
	if (m_pElementSpace)
	{
...
void OnboardEscalatorInSim::CreatePollygon()
{
	...
	ARCPipe arcPath(path,pEscalator->GetWidth());
...
bool OnboardEscalatorInSim::GetLocation( CPoint2008& location ) const
{
