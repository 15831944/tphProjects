...
void LandsideParkingLotLevelInSim::InitGraph(LandsideParkingLotGraph* pDriveGraph)
{
	...
	for(size_t i=0;i<m_vDoors.size();i++)
	{
		...
		for(size_t i=0;i<m_vDriveLanes.size();i++)
		{
			...
			IntersectPathPath2D inters(pLane1->getPath(),path);
			int nCount = inters.Intersects(200);//in range 2 meter
	...
	for(size_t i=0;i<m_vDriveLanes.size();i++)
	{
		...
		for(size_t j=i+1;j<m_vDriveLanes.size();j++)
		{
			...
			IntersectPathPath2D inters(pLane1->getPath(),pLane2->getPath());
			int nCount = inters.Intersects(pLane2->getWidth()*0.5);
			for(int k=0;k<nCount;k++)
			{
				double i1= inters.m_vIntersectPtIndexInPath1[k];
				double d1= pLane1->getPath().GetIndexDist((float)i1);
...
void LandsideParkinglotDriveLaneInSim::CaculateIntesectWithWalkway( CWalkwayInSim* pRes )
{
	...
	IntersectPathPath2D pathint(m_path,walkpath);
	int nCount = pathint.Intersects();
	for(int i=0;i<nCount;i++){
		float dp1 = (float) pathint.m_vIntersectPtIndexInPath1[i];
		float dp2 = (float) pathint.m_vIntersectPtIndexInPath2[i];
