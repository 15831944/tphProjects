...
void BaggageProcUtil::Init( Path* _ptList, DistanceUnit& _dSpeed  )
{
	...
	for( i=0, j=1; j<iCount; ++i, ++j )
	{
		DistanceUnit dDistance = m_vPointAndBags[i].GetPosPoint().distance( m_vPointAndBags[j].GetPosPoint() );
		ElapsedTime costTime ( dDistance / _dSpeed );
		m_vPointAndBags[i].SetCostTime( costTime );
