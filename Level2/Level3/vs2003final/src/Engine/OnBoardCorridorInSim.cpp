...
void OnboardCorridorInSim::CaculateSpans()
{
	...
	for(int i=1;i<center.getCount();i++)
	{
		...
		{
			DistancePointPath3D distPath(center.getPoint(i),left);
			distPath.GetSquared();
			...
			{		
				DistancePointLine3D distline(left.getPoint(ptidx),centerline);
				distline.GetSquared();
