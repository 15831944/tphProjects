...
CCar::~CCar()
{
	...
	for(DOOR ::iterator iter=m_entryDoors.begin();iter!=m_entryDoors.end();++iter)
		delete (*iter);
	m_entryDoors.clear();
...
//	m_preBoardingArea->writeProcessor(procFile);
	...
	procFile.writeLine();
	for(DOOR ::const_iterator iter=m_entryDoors.begin();iter!=m_entryDoors.end();++iter)
	{
...
bool CCar::ComputeDoorInfo(int _nEntryDoorNum, int _nExitDoorNum, int _nExitDoorType , double _dCarLength, double _dPlatFormWidth )
{
	...
	if( _nExitDoorType )
	{
		...
		for( i=0;i<_nExitDoorNum; ++i )
		{
			doorLocationAndQueue[0].DoOffset( dSinglDoorDistance,0,0 );
			doorLocationAndQueue[1].DoOffset( dSinglDoorDistance,0,0 );
			m_exitDoors[i]->initServiceLocation(1,doorLocationAndQueue);
			doorLocationAndQueue[0].setY( basePoint.getY() - PLATFORM_CAR_DISTANCE - ENTRY_DOOR_QUEUE_OFFSET );
			m_exitDoors[i]->initOutConstraint( 2, doorLocationAndQueue );
			doorLocationAndQueue[0].setY( basePoint.getY() );
...
void CCar::CleanData()
{
	...
	for( i=0; i<nExitDoorCount; i++ )
	{
		Processor* pExitDoor = GetExitDoor( i );
		pExitDoor->CleanData();
	}
