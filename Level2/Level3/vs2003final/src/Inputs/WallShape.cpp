...
inline static void  RenderSolidPath(const std::vector<Point> & path_,DistanceUnit width_ ,DistanceUnit height_){
	...
	for(int i=0;i<(int) widthpath.leftpts.size()-1;++i){
		ARCVector3 v1(widthpath.leftpts[i],widthpath.leftpts[i+1]);
		ARCVector3 vnor = v1^ARCVector3(0,0,1);
		vnor.Normalize();
		glNormal3dv(vnor);
		glVertex3dv(widthpath.leftpts[i] + vheight);glVertex3dv(widthpath.leftpts[i]);
		glVertex3dv(widthpath.leftpts[i+1]);glVertex3dv(widthpath.leftpts[i+1] + vheight);
		glNormal3dv(-vnor);
		glVertex3dv(widthpath.rightpts[i]);glVertex3dv(widthpath.rightpts[i] + vheight);
		glVertex3dv(widthpath.rightpts[i+1] + vheight);glVertex3dv(widthpath.rightpts[i+1]);
	}
	if(nptCount<2) return;
	glNormal3dv(widthpath.leftpts[i]- widthpath.leftpts[i-1]);
