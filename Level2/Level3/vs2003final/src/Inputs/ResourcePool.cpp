...
bool CResourcePool::handleRequestIfCan( const ElapsedTime& _time,const PoolType& emPoolType,BOOL bWait)
{
	...
	_pResource->setDestination( m_ResourceRequestList.front().request_point);
	...
	_pResource->setState( Resource_Arrival_Processor );
	_pResource->generateEvent( _time + _pResource->moveTime(),false );
