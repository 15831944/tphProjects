...
int Flight::operator < (const Flight& p_flight) const
{
    ...
    {
        case AirlineField:
            return strcmpi (m_Airline, p_flight.m_Airline) < 0;
        case ArrIDField:
            if (!strcmpi (m_Airline, p_flight.m_Airline))
                return getArrID() < p_flight.getArrID() ;
            else
                return strcmpi (m_Airline, p_flight.m_Airline) < 0;
        case OrigField:
			return strcmpi (getOrigin(), p_flight.getOrigin()) < 0;
		case ArrStopoverField:
			return strcmpi (m_arrStopover, p_flight.m_arrStopover) < 0;
        case ArrTimeField:
        ...
        case DepStopoverField:
			return strcmpi (m_depStopover, p_flight.m_depStopover) < 0;
		case DestField:
			return strcmpi (getDestination(), p_flight.getDestination()) < 0;
        case DepTimeField:
            return getDepTime() < p_flight.getDepTime();
		case GateTimeField:
			return getGateTime() < p_flight.getGateTime();
        case ACTypeField: default:
            return strcmpi (m_AcType, p_flight.m_AcType) < 0;
