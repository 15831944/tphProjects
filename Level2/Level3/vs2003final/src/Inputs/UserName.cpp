...
bool CUserNameDataSet::canModifyUseName(const ProcessorID& _procID, const CUserName& _userName)
{
	...
	for( i= 0; i<(int)vNumList.size(); i++ )
	{
		std::vector<int>::const_iterator iter= std::find( vLeafNum.begin(), vLeafNum.end(), vNumList[i] );
		if( iter == vLeafNum.end() )	// not exist
		{
			strMsg.Format(" The username num:%d don't exist in processor list",vNumList[i] );
			AfxMessageBox( strMsg,  MB_OK|MB_ICONINFORMATION );
			return false;
		}
	}
