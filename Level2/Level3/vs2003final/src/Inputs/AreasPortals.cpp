...
bool CAreas::InitDataBeforeEngine( CPaxFlowConvertor* _pConvertor, CAllPaxTypeFlow* _pAllFlow )
{
	...
	{
		CSinglePaxTypeFlow* _pSingleFlow = _pAllFlow->GetSinglePaxTypeFlowAt( i );
		assert( _pSingleFlow );
...
*/		
	...
	for(i=0; i<static_cast<int>(__tmpIntersectList.size()); i++ )
	{
		if( i%2 == _iContain)
			__tmpIntersectList[i].intersectInfo[0].state = ENTRY_AREA;
		else
			__tmpIntersectList[i].intersectInfo[0].state = LEAVE_AREA;
	}
