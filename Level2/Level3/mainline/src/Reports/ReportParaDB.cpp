...
void CReportParaDB::readObsoleteData ( ArctermFile& p_file )
{
	...
	else if ( p_file.getVersion() >=2.5 && p_file.getVersion() <2.59 ) 
	{
		...
		{
			if( m_vAllReportPara[ii]->GetReportCategory() == ENUM_DISTANCE_REP
				|| m_vAllReportPara[ii]->GetReportCategory() == ENUM_DURATION_REP )
			{
				CReportParameter* pPara = m_vAllReportPara[ii]->GetDecorator();
				assert( pPara );				
				while( CReportParameter* pParaNext = pPara->GetDecorator() )
				{
					pPara = pParaNext;
