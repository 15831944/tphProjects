...
void CDetailFlightScheduleDelayResult::Draw3DChart(CARC3DChart& chartWnd, CParameters *pParameter)
{
	CString strLabel = _T("");
...
void CDetailFlightScheduleDelayResult::GenerateResult(vector<CAirsideFlightDelayReport::FltTypeDelayItem>& fltTypeDelayData,CParameters *pParameter)
{
...
long CDetailFlightScheduleDelayResult::GetMinDelayTime(vector<CAirsideFlightDelayReport::FltTypeDelayItem>& fltTypeDelayData)
{
	...
	for (; iter!=fltTypeDelayData.end(); iter++)
	{
		...
		for (; iterItem!=(*iter).m_vDelayData.end(); iterItem++)
		{
			...
			{
				bInitFlag = false;
				lMinDelayTime = GetScheduleDelayTime(*iterItem);
			}
			else
			{
				long lDelayTime = GetScheduleDelayTime(*iterItem);
				if (lDelayTime < lMinDelayTime)
				{
					lMinDelayTime = lDelayTime;
				}
			}
		}
	}
...
long CDetailFlightScheduleDelayResult::GetScheduleDelayTime(CAirsideFlightDelayReport::FltDelayItem& item)
{
...
long CDetailFlightScheduleDelayResult::GetMaxDelayTime(vector<CAirsideFlightDelayReport::FltTypeDelayItem>& fltTypeDelayData)
{
	vector<CAirsideFlightDelayReport::FltTypeDelayItem>::iterator iter = fltTypeDelayData.begin();
	...
	for (; iter!=fltTypeDelayData.end(); iter++)
	{
		...
		for (; iterItem!=(*iter).m_vDelayData.end(); iterItem++)
		{
			...
			{
				bInitFlag = false;
				lMaxDelayTime = GetScheduleDelayTime(*iterItem);;
			}
			else
			{
				long lDelayTime = GetScheduleDelayTime(*iterItem);
				if (lDelayTime > lMaxDelayTime)
				{
					lMaxDelayTime = lDelayTime;
				}
			}
		}
	}
