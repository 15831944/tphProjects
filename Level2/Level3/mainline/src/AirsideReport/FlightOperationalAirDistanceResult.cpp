...
CFlightOperationalAirDistanceResult::CFlightOperationalAirDistanceResult(void)
{
}
...
void CFlightOperationalAirDistanceResult::GenerateResult(vector<FltOperationalItem>& vResult, CParameters *pParameter)
{
	...
	for (int i=0; i<(int)m_pResultData.size(); i++)
	{
		...
		for (int j=0; j<(int)vResult.size(); j++)
		{
			...
			{
				double dDistance = ARCUnit::ConvertLength(vResult[j].m_fAirDis, ARCUnit::CM, ARCUnit::KM);
...
void CFlightOperationalAirDistanceResult::Draw3DChart(CARC3DChart& chartWnd, CParameters *pParameter)
{
	CString strLabel = _T("");
	...
	strFooter.Format(_T("FLIGHT TYPE VS AIR DISTANCE %s,%s "), pParameter->getStartTime().printTime(), pParameter->getEndTime().printTime());
