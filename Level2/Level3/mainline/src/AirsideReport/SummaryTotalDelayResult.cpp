...
CSummaryTotalDelayResult::CSummaryTotalDelayResult(void)
{
}
...
bool CSummaryTotalDelayResult::PrepareData(std::vector<CAirsideFlightDelayReport::FltTypeDelayItem>& fltDelayData, FlightConstraint& fltConstraint, vector<long>& vDelayTime, CStatisticalTools<double>& statisticalTool)
{
	for (int i=0; i<(int)fltDelayData.size(); i++)
	{
		if (fltConstraint.fits(fltDelayData[i].m_fltCons))
		{
			...
			{
				vDelayTime.push_back((*iter).totalDelayTime);
				statisticalTool.AddNewData(((*iter).totalDelayTime));
			}
		}
	}
...
void CSummaryTotalDelayResult::Draw3DChart(CARC3DChart& chartWnd, CParameters *pParameter)
{
	CString strLabel = _T("");
