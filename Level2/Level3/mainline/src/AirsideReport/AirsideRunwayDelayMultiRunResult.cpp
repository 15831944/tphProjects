    ...
    const mapRunwaySummaryDelay& mapSummaryData, int iType)
{
    ...
    for(; itor!=mapSummaryData.end(); ++itor)
    {
        ...
        if(itor->second.find(*pTimeInterval) == itor->second.end())
            continue;
...
std::vector<TimeInterval*> CAirsideRunwayDelayMultiRunResult::GetDelayedIntervalList(CParameters *parameter)
{
    ...
    {
        mapSummaryDataOfInterval::const_iterator intervalItor = itor->second.begin();
        for(; intervalItor!=itor->second.end(); ++intervalItor)
        {
            if(intervalItor->second.m_nDataCount != 0)
            {
