...
bool CPipeGraphMgr::initGraphMgr( InputTerminal* _pInTerm, int _numOfLayer )
{
	...
	for(int ii=0; ii<_numOfLayer; ii++ )
	{
		m_allPipeGraph[ii].buildBoostGraph();
	}
	return true;
}
...
int CPipeGraphMgr::removeRedundantPoint( CGraphVertexList& _shortestPath )
{
	...
	for(int ii= removeIndexList.size()-1; ii>=0; ii-- )
	{
		vertex = _shortestPath.getItem( removeIndexList[ii] );
		if( vertex.getVertexType() == PIPE2PIPE )
		{
			_shortestPath.getVertexList().erase( _shortestPath.getVertexList().begin() + removeIndexList[ii] );
			iRemoveNum++;
		}
	}
