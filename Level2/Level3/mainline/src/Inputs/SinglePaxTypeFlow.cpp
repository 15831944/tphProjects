...
void CSinglePaxTypeFlow::AddDestProc(const ProcessorID & _sourceProcID,const CFlowDestination& _destProc , bool _bMustEvenPercent )
{
	if( _sourceProcID == _destProc.GetProcID() )
		return;
	PAXFLOWIDGRAPH::iterator iterSource = m_vPaxFlowDigraph.end();
	PAXFLOWIDGRAPH::iterator iterDest = m_vPaxFlowDigraph.end();
	...
	for( ; iter != iterEnd; ++iter )
	{
		...
		if( iterSource != m_vPaxFlowDigraph.end() && iterDest != m_vPaxFlowDigraph.end() )
			break;
	}

	//ASSERT( iterSource );
	bool bDest = false;
	if( iterDest == m_vPaxFlowDigraph.end() )
		bDest = true;

	if( iterSource == m_vPaxFlowDigraph.end() )
	{
...
void CSinglePaxTypeFlow::InsertProceoosorAfter( const ProcessorID& _sourceProcID , const ProcessorID& _insertProcID  )
{
	...
	for( ; iter != iterEnd; ++iter )
	{
		...
		if( iterSource != m_vPaxFlowDigraph.end() && iterNewProc != m_vPaxFlowDigraph.end() )
			break;
	}
...
void CSinglePaxTypeFlow::DeleteProcNode( const ProcessorID& _procID )
{
	...
	PAXFLOWIDGRAPH::iterator iterProc = m_vPaxFlowDigraph.end();
	...
	//ASSERT( iterProc );
	if( iterProc != m_vPaxFlowDigraph.end() )
	{
...
{
	std::vector<ProcessorID>::const_iterator iterFindID = _sourceIDVector.end();
	PAXFLOWIDGRAPH::const_iterator iter = m_vPaxFlowDigraph.begin();
	PAXFLOWIDGRAPH::const_iterator iterEnd = m_vPaxFlowDigraph.end();
//	PrintAllStructure();
	for( ; iter != iterEnd; ++iter )
