...
void CDirectionUtil::ExpandAllProcessInFlow( CSinglePaxTypeFlow* _pFlow )
{
	...
	for( int ii=0; ii<iProcessCount; ++ii )
	{
		CSubFlow* pProcess = pTerm->m_pSubFlowList->GetProcessUnit( vProcessID[ii].GetIDString() );
		ASSERT( pProcess );
		CSinglePaxTypeFlow* pFlowInProcess = pProcess->GetInternalFlow();

		std::vector<ProcessorID> vSourceID;
		_pFlow->GetSourceProc( vProcessID[ii] ,vSourceID );
		...
		for( int j=0; j<iSourceCount; ++j )
		{
			...
			CProcessorDestinationList* pProcessPair = _pFlow->GetFlowPairAt( vProcessID[ ii ] );
			//// TRACE("\n id=%s\n", pProcessPair->GetProcID().GetIDString() );
			int iPairDest = pProcessPair->GetDestCount();
			for(int  kk=0; kk<iLeafCount; ++kk )
			{
				...
				{
					pProcessPair = _pFlow->GetFlowPairAt( vProcessID[ ii ] );// must get pProcessPair here
					...
					_pFlow->AddDestProc( vLeafInProcess[ kk ], tempDest, false );
...
//		_pFlow->PrintAllStructure();
		_pFlow->DeleteProcNode( vProcessID[ ii ] );
//		_pFlow->PrintAllStructure();
...
//						}
	...
	for(int jj=0; jj<iReclaimCount; ++jj )
	{			
		std::vector<ProcessorID>::iterator iterFind = std::find( iterReclaimStart, vPathProc.end(), vReclaimProc[ jj] );
		ASSERT( iterFind != vPathProc.end() );

		std::vector<ProcessorID> vAllReclaimProc;
		CSubFlow* pSubFlow = pTerm->m_pSubFlowList->GetProcessUnit(vReclaimProc[jj].GetIDString());
		if(pSubFlow)//subflow
		{
		...
		{
			vAllReclaimProc.push_back(vReclaimProc[jj]);
		}
