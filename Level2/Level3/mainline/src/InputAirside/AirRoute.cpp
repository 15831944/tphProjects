...
ARWaypoint::ARWaypoint( const ARWaypoint& leftWayPt)
{
    ...
    m_interceptAngle = leftWayPt.m_interceptAngle;
}
...
void ARWaypoint::UpdateData()
{
 ...
 ARWAYPOINTTYPE=%d,ALTITUDE=%f,ANGLE=%f,TONEXTTYPE=%d,DIRECTTYPE=%d,INBOUNDTRACKANGLE=%f,INTERCEPTANGLE=%f\
 WHERE ID = %d"),
...
void ARWaypoint::SaveData(int nAirRouteID)
{
 ...
 (WAYPOINTID, AirRouteID, MinSpeed, MaxSpeed, MinHeight, MaxHeight,\
 DepartFor,HeadingDegree, Headingtype, VisDistance, SEQUENCENUM\
 ARWAYPOINTTYPE,ALTITUDE,ANGLE,TONEXTTYPE,DIRECTTYPE,INBOUNDTRACKANGLE,INTERCEPTANGLE)\
...
{
	CString strSQL  = _T("");
	strSQL.Format(_T("SELECT ID, WAYPOINTID, NAME_L1, NAME_L2, NAME_L3, NAME_L4, \
 LOCKED, MINSPEED, MAXSPEED, MINHEIGHT, MAXHEIGHT,DEPARTFOR,\
 HEADINGDEGREE,HEADINGTYPE,VISDISTANCE, SEQUENCENUM,ARWAYPOINTTYPE,\
 ALTITUDE,ANGLE,TONEXTTYPE,DIRECTTYPE,INBOUNDTRACKANGLE,INTERCEPTANGLE\
 ...
 WHERE (AIRROUTEID = %d) order by SEQUENCENUM"),m_nObjID);
	...
	while (!adoRecordset.IsEOF())
	{
        ...
        int nWaypointType;
        adoRecordset.GetFieldValue(_T("ARWAYPOINTTYPE"),nWaypointType);
        pARwayPoint->SetWaypointType(ARWaypoint::ARWaypointType(nWaypointType));
        double fAltitude;
        adoRecordset.GetFieldValue(_T("ALTITUDE"),fAltitude);
        pARwayPoint->SetAltitude(fAltitude);
        double fAngle;
        adoRecordset.GetFieldValue(_T("ANGLE"),fAngle);
        pARwayPoint->SetAngle(fAngle);
        int nToNextType;
        adoRecordset.GetFieldValue(_T("TONEXTTYPE"),nToNextType);
        pARwayPoint->SetNextPtType(ARWaypoint::ToNextType(nToNextType));
        int nDirectType;
        adoRecordset.GetFieldValue(_T("DIRECTTYPE"),nDirectType);
        pARwayPoint->SetDirectTpye(ARWaypoint::DirectType(nDirectType));
        double fInBoundTrackAngle;
        adoRecordset.GetFieldValue(_T("INBOUNDTRACKANGLE"), fInBoundTrackAngle);
        pARwayPoint->SetInboundTrackAngle(fInBoundTrackAngle);
        double fInterceptAngle;
        adoRecordset.GetFieldValue(_T("INTERCEPTANGLE"), fInterceptAngle);
        pARwayPoint->SetInterceptAngle(fInterceptAngle);
