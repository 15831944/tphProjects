...
void AirsideFlighQueue::AddFlight( AirsideFlightInSim* pFlight, ElapsedTime time )
{
	TY_FlightList::iterator itrFind = std::find(m_vFlights.begin(),m_vFlights.end(),pFlight);
	if(itrFind == m_vFlights.end())
	{
		m_vFlights.push_back(pFlight);
		m_lastTime = time;
	}
}

void AirsideFlighQueue::RemoveFlight( AirsideFlightInSim* pFlight, ElapsedTime time )
{
	TY_FlightList::iterator itrFind = std::find(m_vFlights.begin(),m_vFlights.end(),pFlight);
	if(itrFind!= m_vFlights.end() )
	{
		m_vFlights.erase(itrFind);
		m_lastTime = time;
	}
}

bool AirsideFlighQueue::IsFlightInQueue( AirsideFlightInSim*pFlight ) const
{
	return std::find(m_vFlights.begin(),m_vFlights.end(),pFlight) != m_vFlights.end();
}
