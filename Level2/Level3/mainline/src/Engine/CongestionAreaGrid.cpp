...
bool GetPoint(const Point& nearPt,const Point& pCurrentPt,const Point& pDestPt,Point& resultPt)
{
	...
	IntersectPathPolygon2D linePath;
	linePath.Intersects(path1,path2);
...
Point CCongestionAreaGrid::GetNearestPoint(const Point& pCurrentPt,const Point& pDestPt)
{
	...
	nearPt = vDistPoint[0];
	for (size_t ii = 1; ii < vDistPoint.size(); ii++)
	{
		CCongestionAreaCell* pCell = GetCell(vDistPoint[ii]);
		if (pCell && pDestPt.distance(vDistPoint[ii]) < pDestPt.distance(nearPt))
		{
			nearPt = vDistPoint[ii];
		}
	}
