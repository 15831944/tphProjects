////// TreeCtrlRunwayExit.cpp : implementation file
//////
//////+++++++++++++++++++++++++++++++++++++++
////#include "stdafx.h"
...
////// CTreeCtrlRunwayExit
////
////IMPLEMENT_DYNAMIC(CTreeCtrlRunwayExit, CTreeCtrl)
////CTreeCtrlRunwayExit::CTreeCtrlRunwayExit()
////{
////}
////
////CTreeCtrlRunwayExit::~CTreeCtrlRunwayExit()
...
////
////
////BEGIN_MESSAGE_MAP(CTreeCtrlRunwayExit, CTreeCtrl)
////END_MESSAGE_MAP()
////
//////+++++++++++++++++++++++++++++++++++++++++++++

// CTreeCtrlRunwayExit message handlers
...
BEGIN_MESSAGE_MAP(CTreeCtrlRunwayExit, CTreeCtrl)
	//{{AFX_MSG_MAP(CTreeCtrlRunwayExit)
...
HTREEITEM CTreeCtrlRunwayExit::InsertItem(LPCTSTR lpszItem, COOLTREE_NODE_INFO cni,BOOL bCheckOnOrRadioOn, 
...
CWnd* CTreeCtrlRunwayExit::CreateEditWnd(NODE_EDIT_TYPE net)
{
	CWnd* pNewWnd=NULL;
	switch(net)
	{
...
COOLTREE_NODE_INFO* CTreeCtrlRunwayExit::GetItemNodeInfo(HTREEITEM hItem)
{
...
int CTreeCtrlRunwayExit::IsCheckItem(HTREEITEM hItem)
{
	int nImage,nReturn;
...
BOOL CTreeCtrlRunwayExit::IsEnableItem(HTREEITEM hItem)
{
	int nImage;
	if(nImage=GetItemImage(hItem))
	{
...
void CTreeCtrlRunwayExit::PreSubclassWindow() 
{
...
void CTreeCtrlRunwayExit::SetCheckStatus(HTREEITEM hItem, BOOL bCheck,BOOL bRecursive)
{
...
void CTreeCtrlRunwayExit::SetEnableStatus(HTREEITEM hItem, BOOL bEnable)
{
	if (m_EnableMutiTree == TRUE)
		return;
...
TREENODE_IMAGE_INDEX_TABLE CTreeCtrlRunwayExit::GetItemImage(HTREEITEM hItem)
{
...
void CTreeCtrlRunwayExit::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CTreeCtrlRunwayExit::DoCheck(HTREEITEM hItem)
{
	SetCheckStatus(hItem,!IsCheckItem(hItem));
}

void CTreeCtrlRunwayExit::DoRadio(HTREEITEM hItem)
{
	SetRadioStatus(hItem,TRUE);
}

void CTreeCtrlRunwayExit::SetRadioStatus(HTREEITEM hItem,BOOL bRadio)
{
...
BOOL CTreeCtrlRunwayExit::IsRadioItem(HTREEITEM hItem)
{
	return IsCheckItem(hItem)==1;
}

void CTreeCtrlRunwayExit::AutoAdjustItemEnableStatus(HTREEITEM hItem)
{
	if (m_EnableMutiTree == TRUE)
		return;
...
void CTreeCtrlRunwayExit::AutoAdjustRadioStatus(HTREEITEM hItem)
{
...
void CTreeCtrlRunwayExit::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
...
void CTreeCtrlRunwayExit::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CTreeCtrlRunwayExit::ShowEditSpinWnd(HTREEITEM hItem,COOLTREE_NODE_INFO* pCNI)
{
	CWnd* pWnd=NULL;
	if(pCNI->pEditWnd)
	{
	...
	if(pCNI->net==NET_EDITSPIN_WITH_VALUE||pCNI->net==NET_EDIT_WITH_VALUE)
	{
		...
		if(GetItemRect(hItem,rect,TRUE))
		{
			...
			else
			{
				...
				m_pRunwayExitPercentItem = (RunwayExitPercentItem*)GetItemData(hItem);
				strItemData.Format("%d",(int)(m_pRunwayExitPercentItem->m_fPercent));			
			}
...
void CTreeCtrlRunwayExit::ShowEditWnd(HTREEITEM hItem,COOLTREE_NODE_INFO* pCNI,BOOL bCreateOnly)
{
	if(bCreateOnly)
	{
...
LRESULT CTreeCtrlRunwayExit::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void CTreeCtrlRunwayExit::ShowComboBoxWnd(HTREEITEM hItem, COOLTREE_NODE_INFO *pCNI,BOOL bEditable)
{
	CWnd* pWnd=NULL;
	if(!bEditable)
	{
...
// Implementation of CTreeCtrlRunwayExit::CTreeComboBox class
BEGIN_MESSAGE_MAP(CTreeCtrlRunwayExit::CTreeComboBox, CComboBox)
	...
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
void CTreeCtrlRunwayExit::CTreeComboBox::OnKillFocus() 
{
	ShowWindow( SW_HIDE );
...
void CTreeCtrlRunwayExit::CTreeComboBox::OnSelChange(NMHDR* pNMHDR, LRESULT* pResult)
{
	ShowWindow( SW_HIDE );
	m_nIndexSeled=GetCurSel();
	CString strValue;
...
void CTreeCtrlRunwayExit::OnKillfocusEdit1() 
{
	// TODO: Add your control notification handler code here
...
CWnd* CTreeCtrlRunwayExit::GetEditWnd(HTREEITEM hItem)
{
	CWnd* pWnd=NULL;
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(!pCNI)return NULL;
...
CWnd* CTreeCtrlRunwayExit::GetEditWnd(NODE_EDIT_TYPE net)
{
	CWnd* pWnd=NULL;
	switch(net)
	{
...
void CTreeCtrlRunwayExit::SetItemNodeInfo(HTREEITEM hItem, COOLTREE_NODE_INFO cni)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
		*pCNI=cni;
...
DWORD CTreeCtrlRunwayExit::GetItemData(HTREEITEM hItem)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
...
BOOL CTreeCtrlRunwayExit::SetItemData(HTREEITEM hItem, DWORD dwData)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
	{
...
void CTreeCtrlRunwayExit::SelectFromText(CComboBox *pCB, CString strValue)
{
...
void CTreeCtrlRunwayExit::InitNodeInfo(CWnd* pParent,COOLTREE_NODE_INFO& CNI,BOOL bVerify)
{
	CNI.bEnable=TRUE;
	CNI.dwItemData=NULL;
	CNI.fMaxValue=100;
...
void CTreeCtrlRunwayExit::DoEdit(HTREEITEM hItem)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(!pCNI) return;
	switch(pCNI->net)
...
void CTreeCtrlRunwayExit::SetItemValueRange(HTREEITEM hItem,int nMin, int nMax, BOOL bVerify)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
	{
...
void CTreeCtrlRunwayExit::OnPaint() 
{
...
void CTreeCtrlRunwayExit::SetItemBold(HTREEITEM hItem,BOOL bBold,BOOL bRedraw)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
	{
...
void CTreeCtrlRunwayExit::OnRclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	CPoint point;
	GetCursorPos(&point);
...
BOOL CTreeCtrlRunwayExit::IsCheckBox(HTREEITEM hItem)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
...
void CTreeCtrlRunwayExit::SetChect2Status(HTREEITEM hItem)
{
	if(IsEnableItem(hItem))
...
void CTreeCtrlRunwayExit::AutoAdjustCheck2Status(HTREEITEM hItem)
{
...
int CTreeCtrlRunwayExit::GetCmbBoxCurSel(HTREEITEM hItem)
{
...
void CTreeCtrlRunwayExit::OnSelchangeCombo()
{
...
HTREEITEM CTreeCtrlRunwayExit::GetSelectedItem()
{
	return m_hEditedItem;
}

BOOL CTreeCtrlRunwayExit::DeleteItem( HTREEITEM hItem )
{
