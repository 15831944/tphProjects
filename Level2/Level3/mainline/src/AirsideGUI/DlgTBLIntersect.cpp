// DlgTBLIntersect.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlgTBLIntersect, CPropertyPage)
CDlgTBLIntersect::CDlgTBLIntersect(int nProjID, TakeoffClearanceItem* pItem)
	: CPropertyPage(CDlgTBLIntersect::IDD)
	, m_nProjID(nProjID)
	, m_nDistApproach(0)
	, m_nTimeApproach(0)
	, m_nTimeSlot(0)
{
	m_pTBLIntersect = (TakeoffBehindLandingIntersectingConvergingRunway*)pItem;
...
CDlgTBLIntersect::~CDlgTBLIntersect()
{
...
void CDlgTBLIntersect::DoDataExchange(CDataExchange* pDX)
{
...
void CDlgTBLIntersect::InitListCtrl()
{
...
void CDlgTBLIntersect::InitComboBox()
{
	...
	FlightClassificationBasisType emType = 
		m_pTBLIntersect->GetCurrentFltClassificationType();
	m_wndCmbBasis.SetCurSel((int)emType);
}

void CDlgTBLIntersect::InitEditBox()
{

	m_nDistApproach = m_pTBLIntersect->GetDistTakeoffApproach();
	m_nTimeApproach = m_pTBLIntersect->GetTimeTakeoffAproach();
	m_nTimeSlot = m_pTBLIntersect->GetTimeTakeoffSlot();

	if(m_pTBLIntersect->IsDistTakeoffApproach())
...
void CDlgTBLIntersect::ResetListCtrl()
{
	...
	m_pTBLIntersect->SetCurrentFlightClassificationBasisType(fltBasisType);

	TakeoffClearanceItem::TakeoffClearanceBasisVector vect;
	m_pTBLIntersect->GetItemsByType(vect, fltBasisType);
...
void CDlgTBLIntersect::GetValueBeforSave()
{
	UpdateData(TRUE);

	m_pTBLIntersect->SetDistTakeoffApproach(m_nDistApproach);
	m_pTBLIntersect->SetTimeTakeoffAproach(m_nTimeApproach);
	m_pTBLIntersect->SetTimeTakeoffSlot(m_nTimeSlot);

	m_pTBLIntersect->SetDistTakeoffApproachFlag(IsDlgButtonChecked(IDC_RADIO5) == BST_CHECKED);

}



BEGIN_MESSAGE_MAP(CDlgTBLIntersect, CPropertyPage)
...
// CDlgTBLIntersect message handlers

BOOL CDlgTBLIntersect::OnInitDialog()
{
...
void CDlgTBLIntersect::OnCbnSelchangeComboBasis()
{
	ResetListCtrl();
}

void CDlgTBLIntersect::OnLvnEndlabeleditListBasis(NMHDR *pNMHDR, LRESULT *pResult)
{
...
AircraftClassifications* CDlgTBLIntersect::GetAircraftClsByType(FlightClassificationBasisType fltBasisType)
{
	switch(fltBasisType) {
	case WingspanBased:
...
void CDlgTBLIntersect::OnCancel()
{
	return CPropertyPage::OnCancel();
}
void CDlgTBLIntersect::OnOK()
{
	GetValueBeforSave();
