...
/////////////////////////////////////////////////////////////////////////////
// CVehiclePoolsAndDeploymentCoolTree
CVehiclePoolsAndDeploymentCoolTree::CVehiclePoolsAndDeploymentCoolTree()
{
	m_pWndEdit=NULL;
	m_pWndEditSpin=NULL;
	m_pWndComboBox=NULL;
...
BEGIN_MESSAGE_MAP(CVehiclePoolsAndDeploymentCoolTree, CTreeCtrl)
	//{{AFX_MSG_MAP(CVehiclePoolsAndDeploymentCoolTree)
...
/////////////////////////////////////////////////////////////////////////////
// CVehiclePoolsAndDeploymentCoolTree message handlers

HTREEITEM CVehiclePoolsAndDeploymentCoolTree::InsertItem(LPCTSTR lpszItem, COOLTREE_NODE_INFO cni,BOOL bCheckOnOrRadioOn, 
...
CWnd* CVehiclePoolsAndDeploymentCoolTree::CreateEditWnd(NODE_EDIT_TYPE net)
{
	CWnd* pNewWnd=NULL;
	switch(net)
	{
...
COOLTREE_NODE_INFO* CVehiclePoolsAndDeploymentCoolTree::GetItemNodeInfo(HTREEITEM hItem)
{
...
int CVehiclePoolsAndDeploymentCoolTree::IsCheckItem(HTREEITEM hItem)
{
	int nImage,nReturn;
...
BOOL CVehiclePoolsAndDeploymentCoolTree::IsEnableItem(HTREEITEM hItem)
{
	int nImage;
	if(nImage=GetItemImage(hItem))
	{
...
void CVehiclePoolsAndDeploymentCoolTree::PreSubclassWindow() 
{
...
void CVehiclePoolsAndDeploymentCoolTree::SetCheckStatus(HTREEITEM hItem, BOOL bCheck,BOOL bRecursive)
{
...
void CVehiclePoolsAndDeploymentCoolTree::SetEnableStatus(HTREEITEM hItem, BOOL bEnable)
{
	if (m_EnableMutiTree == TRUE)
		return;
...
TREENODE_IMAGE_INDEX_TABLE CVehiclePoolsAndDeploymentCoolTree::GetItemImage(HTREEITEM hItem)
{
...
void CVehiclePoolsAndDeploymentCoolTree::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CVehiclePoolsAndDeploymentCoolTree::DoCheck(HTREEITEM hItem)
{
	SetCheckStatus(hItem,!IsCheckItem(hItem));
}

void CVehiclePoolsAndDeploymentCoolTree::DoRadio(HTREEITEM hItem)
{
	SetRadioStatus(hItem,TRUE);
}

void CVehiclePoolsAndDeploymentCoolTree::SetRadioStatus(HTREEITEM hItem,BOOL bRadio)
{
...
BOOL CVehiclePoolsAndDeploymentCoolTree::IsRadioItem(HTREEITEM hItem)
{
	return IsCheckItem(hItem)==1;
}

void CVehiclePoolsAndDeploymentCoolTree::AutoAdjustItemEnableStatus(HTREEITEM hItem)
{
	if (m_EnableMutiTree == TRUE)
		return;
...
void CVehiclePoolsAndDeploymentCoolTree::AutoAdjustRadioStatus(HTREEITEM hItem)
{
...
void CVehiclePoolsAndDeploymentCoolTree::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
...
void CVehiclePoolsAndDeploymentCoolTree::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CVehiclePoolsAndDeploymentCoolTree::ShowEditSpinWnd(HTREEITEM hItem,COOLTREE_NODE_INFO* pCNI)
{
	CWnd* pWnd=NULL;
	if(pCNI->pEditWnd)
	{
...
void CVehiclePoolsAndDeploymentCoolTree::ShowEditWnd(HTREEITEM hItem,COOLTREE_NODE_INFO* pCNI,BOOL bCreateOnly)
{
	if(bCreateOnly)
	{
...
LRESULT CVehiclePoolsAndDeploymentCoolTree::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
void CVehiclePoolsAndDeploymentCoolTree::ShowComboBoxWnd(HTREEITEM hItem, COOLTREE_NODE_INFO *pCNI,BOOL bEditable)
{
	CWnd* pWnd=NULL;
	if(!bEditable)
	{
...
// Implementation of CVehiclePoolsAndDeploymentCoolTree::CTreeComboBox class
BEGIN_MESSAGE_MAP(CVehiclePoolsAndDeploymentCoolTree::CTreeComboBox, CComboBox)
...
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
void CVehiclePoolsAndDeploymentCoolTree::CTreeComboBox::OnKillFocus() 
{
	ShowWindow( SW_HIDE );
...
void CVehiclePoolsAndDeploymentCoolTree::CTreeComboBox::OnSelChange(NMHDR* pNMHDR, LRESULT* pResult)
{
	ShowWindow( SW_HIDE );
	m_nIndexSeled=GetCurSel();
	CString strValue;
...
DWORD_PTR CVehiclePoolsAndDeploymentCoolTree::CTreeComboBox::GetItemData(int nIndex )
{
	return CComboBox::GetItemData(nIndex);
}

void CVehiclePoolsAndDeploymentCoolTree::OnKillfocusEdit1() 
{
	// TODO: Add your control notification handler code here
...
CWnd* CVehiclePoolsAndDeploymentCoolTree::GetEditWnd(HTREEITEM hItem)
{
	CWnd* pWnd=NULL;
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(!pCNI)return NULL;
...
CWnd* CVehiclePoolsAndDeploymentCoolTree::GetEditWnd(NODE_EDIT_TYPE net)
{
	CWnd* pWnd=NULL;
	switch(net)
	{
...
void CVehiclePoolsAndDeploymentCoolTree::SetItemNodeInfo(HTREEITEM hItem, COOLTREE_NODE_INFO cni)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
		*pCNI=cni;
...
DWORD CVehiclePoolsAndDeploymentCoolTree::GetItemData(HTREEITEM hItem)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
...
BOOL CVehiclePoolsAndDeploymentCoolTree::SetItemData(HTREEITEM hItem, DWORD dwData)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
	{
...
void CVehiclePoolsAndDeploymentCoolTree::SelectFromText(CComboBox *pCB, CString strValue)
{
...
void CVehiclePoolsAndDeploymentCoolTree::InitNodeInfo(CWnd* pParent,COOLTREE_NODE_INFO& CNI,BOOL bVerify)
{
	CNI.bEnable=TRUE;
	CNI.dwItemData=NULL;
	CNI.fMaxValue=100;
...
void CVehiclePoolsAndDeploymentCoolTree::DoEdit(HTREEITEM hItem)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(!pCNI) return;
	switch(pCNI->net)
	...
	case NET_NORMAL:
		OnClick(NULL,NULL);
		GetParent()->SendMessage(UM_CEW_NET_NORMAL,(WPARAM)hItem,(LPARAM)pCNI);
		break;
	case NET_EDIT:
	case NET_EDIT_WITH_VALUE:
	case NET_EDIT_FLOAT:
...
void CVehiclePoolsAndDeploymentCoolTree::SetItemValueRange(HTREEITEM hItem,int nMin, int nMax, BOOL bVerify)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
	{
...
void CVehiclePoolsAndDeploymentCoolTree::OnPaint() 
{
...
void CVehiclePoolsAndDeploymentCoolTree::SetItemBold(HTREEITEM hItem,BOOL bBold,BOOL bRedraw)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
	if(pCNI)
	{
...
void CVehiclePoolsAndDeploymentCoolTree::OnRclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
	CPoint point;
	GetCursorPos(&point);
...
BOOL CVehiclePoolsAndDeploymentCoolTree::IsCheckBox(HTREEITEM hItem)
{
	COOLTREE_NODE_INFO* pCNI=GetItemNodeInfo(hItem);
...
void CVehiclePoolsAndDeploymentCoolTree::SetChect2Status(HTREEITEM hItem)
{
	if(IsEnableItem(hItem))
...
void CVehiclePoolsAndDeploymentCoolTree::AutoAdjustCheck2Status(HTREEITEM hItem)
{
...
int CVehiclePoolsAndDeploymentCoolTree::GetCmbBoxCurSel(HTREEITEM hItem)
{
...
DWORD_PTR CVehiclePoolsAndDeploymentCoolTree::GetCurSelCmbBoxItemData(HTREEITEM hItem)
{
	CWnd* pWnd=GetEditWnd(hItem);
	CTreeComboBox* pTCB=(CTreeComboBox*)pWnd;
...
void CVehiclePoolsAndDeploymentCoolTree::OnSelchangeCombo()
{
...
HTREEITEM CVehiclePoolsAndDeploymentCoolTree::GetSelectedItem()
{
	return m_hEditedItem;
}

BOOL CVehiclePoolsAndDeploymentCoolTree::SelectItem(HTREEITEM hItem)
{
	if (NULL == hItem)
	{
		return FALSE;
...
BOOL CVehiclePoolsAndDeploymentCoolTree::DeleteItem( HTREEITEM hItem )
{
