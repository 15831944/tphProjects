...
bool SolveTri (int size, Real* A, Real* B, Real* C,
								   Real* R, Real* U)
{
	if (B[0] == (Real)0)
	{
...
mA(values)
{
	//assert(numSamples >= 2 && times && values, "Invalid input\n");
												...
												Real slopeLast)
{
	...
	bool solved = SolveTri(numSegmentsM1,&delta[2],
...
void NaturalSpline3D::buildClampe( CPoint2008* pInput,int nCount, const CPoint2008& slope1, const CPoint2008& slope2 )
{
	clear();
	
	
	t = new1<double>(nCount);
	double* vx = new1<double>(nCount);
	double * vy = new1<double>(nCount);
	double* vz = new1<double>(nCount);
	...
	xspline = new NaturalSpline1d(nCount,t,vx,slope1.getX(),slope2.getX());
	yspline = new NaturalSpline1d(nCount,t,vy,slope1.getY(),slope2.getY());
	zspline = new NaturalSpline1d(nCount,t,vz,slope1.getZ(),slope2.getZ());
	
	
}

bool NaturalSpline3D::GetPoint( double t, CPoint2008& p ) const
{
	if(xspline && yspline && zspline)
	{
		p.setX( xspline->GetFunction(t) );
		p.setY( yspline->GetFunction(t) );
		p.setZ( zspline->GetFunction(t) );
		return true;
	}
	return false;
}
...
{
	xspline=yspline=zspline=0;
	t= 0;
}

void NaturalSpline3D::clear()
{
	delete xspline;
	delete yspline;
	delete zspline;
