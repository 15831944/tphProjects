...
#define MAT_STAIR _T("processor/stair")
using namespace Ogre;

void CRenderStairProc3D::Update3D( CProcessor2Base* pProc)
{
	ProcessorProp* pProcProp = pProc->GetProcessorProp();
	ASSERT(pProcProp && pProcProp->getProcessorType() == StairProc);
	ExtraProcProp* pExtraProp = pProcProp->GetExtraProp();
	ASSERT(pExtraProp && pExtraProp->GetPropType() == ExtraProcProp::Stair_Prop_Type);
	StairProp* pStair = (StairProp*)pExtraProp;
	...
	if(pdp.bDisplay[PDP_DISP_SHAPE])
	{
		...
		Ogre::ColourValue clr = OGRECOLOR(cDisplay);
...
void CRenderStairProc3D::DrawStairTexturePath( Ogre::ManualObject* pObj,const Ogre::String& texmat,const Ogre::ColourValue& clr, const ARCPath3&path ,double dWidth, double dSideHeight, bool bLoop/* = false*/ )
{
...
void CRenderStairProc3D::UpdateSelected( bool b )
{

}

void CRenderStairProc3D::Setup3D( CProcessor2Base* pProc, int idx )
{
