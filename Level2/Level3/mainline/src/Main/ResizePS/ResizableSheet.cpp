...
void CResizableSheet::SavePage()
{
	...
	if(!handle.LoadFile(regReader.m_Path.m_AppPath))
	      return ;
	handle.SetCurrentGroup(m_sSection) ;
	TCHAR ch[1024] = {0} ;
	itoa(page,ch,10);
	handle.SetData(ACTIVEPAGE,ch) ;
	handle.SaveData(regReader.m_Path.m_AppPath) ;*/
	((CTermPlanApp*)AfxGetApp())->WriteProfileInt(m_sSection, ACTIVEPAGE, page);
...
void CResizableSheet::LoadPage()
{
   ...
   int page = ((CTermPlanApp*)AfxGetApp())->GetProfileInt(m_sSection, ACTIVEPAGE, 0);
	//int page = 0 ;
	//CReadRegister regReader ;
	//if(!regReader.ReadRegisterPath(CString()))
	//	return  ;
	...
	//handle.SetCurrentGroup(m_sSection) ;
	//CString str_page ;
	//if(!handle.GetData(ACTIVEPAGE,str_page))
	//	page = 0 ;
	//else
	//	page = atoi(str_page) ;
	if (m_bSavePage)
