...
void CAirsideStopSignDlg::CalculateStrechID(void)
{
	...
	for (int i=0; i<(int)m_vStretchPath.first.size(); i++)
	{
		DistancePointPath3D  pDistance;// = new DistancePointPath3D();
		DistanceUnit disTance = pDistance.GetSquared(position,m_vStretchPath.second[i].GetStretch()->GetPath());
