...
CThumbnailsList2::CThumbnailsList2()
{
}
...
BEGIN_MESSAGE_MAP(CThumbnailsList2, CListCtrl)
	//{{AFX_MSG_MAP(CThumbnailsList2)
	ON_WM_MOUSEMOVE()
	ON_WM_KILLFOCUS()
	ON_WM_CREATE()
	ON_WM_LBUTTONDBLCLK()
...
void CThumbnailsList2::CreateThumnailImageList()
{
...
/////////////////////////////////////////////////////////////////////////////
// CThumbnailsList2 message handlers





void CThumbnailsList2::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	//{
		...
		if(nSeledIndex!=-1)
		{
			...
			((CListWnd*)m_pParent)->DisplayPreviewCtrl(this);
		}
	//}
	CListCtrl::OnMouseMove(nFlags, point);
}

void CThumbnailsList2::DrawThumbnails(int _nSet,int nShapeRelationFlag /*= -1*/)
{
	/*
	CBitmap*    pImage = NULL;	
	...
	*/
	...
	if(-1 == _nSet)
		nImgCount = SHAPESMANAGER->GetShapeSetCount();  // we want the shape set thumbnails
	else
	{
		if(nShapeRelationFlag == -1)
			nImgCount = SHAPESMANAGER->GetShapeCount(_nSet);
		else 
			nImgCount = 1;
	}
	...
	for( int _i=0; _i<nImgCount; _i++) {
		...
		*/
		...
		HBITMAP hBitmapOrig;
		if(-1 == _nSet)
			hBitmapOrig = SHAPESMANAGER->GetShapeImgBitmap(_i, 0);
		else
		{
			if(nShapeRelationFlag == -1)
				hBitmapOrig = SHAPESMANAGER->GetShapeImgBitmap(_nSet, _i);
			else
				hBitmapOrig = SHAPESMANAGER->GetShapeImgBitmap(_nSet, 0);
		}
		...
		*/
		...
		if(-1 == _nSet)
			InsertItem(_i, SHAPESMANAGER->GetShapeSetName(_i), _i);
		else
		{
			if(nShapeRelationFlag == -1)
				InsertItem(_i, SHAPESMANAGER->GetShapeName(_nSet, _i), _i);
			else
				InsertItem(_i, SHAPESMANAGER->GetShapeSetName(_nSet), _i);
		}
...
void CThumbnailsList2::OnKillFocus(CWnd* pNewWnd) 
{
...
int CThumbnailsList2::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CThumbnailsList2::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	int nSelectedItem=HitTest(point);
	if(nSelectedItem!=-1)
	{
		m_nSelectedItem = nSelectedItem;
		((CListWnd*)GetParent())->CompleteSelect(this); 
		m_nPrevSeledIndex=-1;
	}
...
int CThumbnailsList2::GetSelectedItem()
{
	return m_nSelectedItem;
}

int CThumbnailsList2::SetSelectedItem(int _n)
{
