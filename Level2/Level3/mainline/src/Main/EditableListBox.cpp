	...
	if( m_ceEdit.GetSafeHwnd() != NULL )
	{
		...
		if( bCommitText && m_iItemBeingEdited != -1 )
		{
			...
			if( csOldItem.Compare( csNewItem ) != 0 )
			{
				...
				DWORD_PTR pdwItemData = GetItemData(m_iItemBeingEdited);
				...
				SetItemData(m_iItemBeingEdited,pdwItemData);
...
void CEditableListBox::OnLButtonDblClk(UINT nFlags, CPoint point)
{
