...
void CProcPropDlg::BuildAllSubProcessorID()
{
		...
		for(int i=0; iter!=iterLast;++i,++iter)
		{
			...
			for(int j=0;j<static_cast<int>((*iter)->m_entryDoors.size());++j)
			{
			  tempStr.Format("%d",j);
...
{
	CString strFileName = "";
	CFileDialog fileDlg(TRUE, _T("bmp"),NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Bitmap Files (*.bmp)|*.bmp|Jpeg Files(*.jpg)|*.jpg|All Files (*.*)|*.*||"), NULL, 0, FALSE );
