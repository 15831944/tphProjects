...
void CAirsideRepControlView::OnButtonReportDefault()
{
	...
	if (GetReportType() != Airside_FlightConflict)
	{
		m_pParameter->LoadParameterFile();
	}
	...
	{
		ArctermFile  _file ;
		 pTermPlanDoc->GetARCReportManager().GetAirsideReportManager()->InitMultipleRunReport(rpType);
		CString strFilePath = pTermPlanDoc->GetARCReportManager().GetAirsideReportManager()->GetReportFilePath(rpType);
...
void CAirsideRepControlView::OnButtonApply()
{
	...
	std::vector<int> vReportRun;
	if(m_pParameter->GetReportRuns(vReportRun) && m_pParameter->GetEnableMultiRun())
	{
		if (vReportRun.size() <= 1)
		{
			MessageBox(_T("Must select two runs at least."), _T("Warning"), MB_OK);
			return;
		}
	}
...
LRESULT CAirsideRepControlView::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    else if(message == UM_CEW_STATUS_CHANGE)
    {
        if(GetReportType() == Airside_FlightDelay ||
            ...
            GetReportType() == Airside_AircraftOperational)
        {
            HTREEITEM hSelItem = (HTREEITEM)wParam;
