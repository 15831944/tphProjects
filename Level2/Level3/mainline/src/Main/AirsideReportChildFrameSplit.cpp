// AirsideReportChildFrameSplit.cpp : implementation file
//
...
// CAirsideReportChildFrameSplit

IMPLEMENT_DYNCREATE(CAirsideReportChildFrameSplit, CMDIChildWnd)

CAirsideReportChildFrameSplit::CAirsideReportChildFrameSplit()
{
}
...
BOOL CAirsideReportChildFrameSplit::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	if( !m_wndSplitter2.CreateView( 0, 0,			// row 0, col 0
		RUNTIME_CLASS( CAirsideRepControlView ), 
		size, pContext ) )
	{
	...
	if( !m_wndSplitter1.CreateView( 0, 1,				// row 0, col 1
		RUNTIME_CLASS( CAirsideReportGraphView ), 
...
BEGIN_MESSAGE_MAP(CAirsideReportChildFrameSplit, CMDIChildWnd)
END_MESSAGE_MAP()


// CAirsideReportChildFrameSplit message handlers

void CAirsideReportChildFrameSplit::OnUpdateFrameTitle(BOOL bAddToTitle)
{
	...
	if(bAddToTitle)
	{
		...
		if (pDocument == NULL)
			...
			lstrcpy(szText, pDocument->GetTitle() + " - " + ((CTermPlanDoc*)pDocument)->GetARCReportManager().GetAirsideReportTitle() );
...
void CAirsideReportChildFrameSplit::OnUpdateFrameMenu(BOOL bActive, CWnd* pActivateWnd, HMENU hMenuAlt)
{
