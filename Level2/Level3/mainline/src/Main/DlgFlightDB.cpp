...
void CDlgFlightDB::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_GROUP, m_FlightGroup);
}
...
void CDlgFlightDB::OnBnClickedOk()
{
 ...
 void CDlgFlightDB::OnBnClickedExport()
 {
	CFileDialog filedlg( FALSE,"frp", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
		"FlightGroup File (*.frp)|*.frp;*.fpr|All type (*.*)|*.*|", NULL, 0, FALSE );
 ...
 LRESULT CDlgFlightDB::DefWindowProc( UINT message,WPARAM wParam,LPARAM lParam )
 {
 ...
 void CDlgFlightDB::DoResize( int nDelta )
 {
	 CSplitterControl::ChangeHeight(&m_listCtrlSchdList,nDelta,CW_TOPALIGN);
	 CSplitterControl::ChangeHeight(&m_wndListBox,-nDelta,CW_BOTTOMALIGN);
	 CSplitterControl::ChangeHeight(&m_wndListCtrl,-nDelta,CW_BOTTOMALIGN);

	 CRect rect;
	 m_FlightGroup.GetWindowRect(rect);
	 ScreenToClient(rect);
	 m_FlightGroup.SetWindowPos(NULL,rect.left,rect.top+nDelta,0,0,SWP_NOSIZE);

	 m_wndToolBar.GetWindowRect(rect);
	 ScreenToClient(rect);
	 m_wndToolBar.SetWindowPos(NULL,rect.left,rect.top+nDelta,0,0,SWP_NOSIZE|SWP_NOACTIVATE|SWP_NOZORDER);
