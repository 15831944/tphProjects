...
BEGIN_MESSAGE_MAP(CARCportTabCtrl, CTabCtrl)
	ON_WM_SIZE()
...
void CARCportTabCtrl::SetRealTitle( const CString& szSchedule, const CString& szRoster )
{
	m_strRosterTitle = szRoster;
	m_strScheduleTitle = szSchedule;

	UpdateTabLabel(0,m_strScheduleTitle);
	UpdateTabLabel(1,m_strRosterTitle);
}

void CARCportTabCtrl::UpdateTabLabel( int iTab, const CString& strLable )
{
	TCHAR szText[256];
	...
	GetItem(iTab, &tci);
	strcpy_s(szText, 256, strLable);
	SetItem(iTab, &tci);
}

BOOL CARCportTabCtrl::Create(CWnd* pWnd, const CRect& rect)
{
	if (!CTabCtrl::Create(WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_TABSTOP, rect, pWnd, IDC_TAB_PANEL))
		return FALSE;
...
//	AddToolTip(1, m_strRosterTitle);
	...
	if (InsertItem(1,m_strRosterTitle) == -1)
		return FALSE;
...
BOOL CARCportTabCtrl::InitScheduleList()
{
	...
	if (InsertItem(0,m_strScheduleTitle) == -1)
		return FALSE;
...
void CARCportTabCtrl::OnSelChangeTabList( NMHDR* pNMHDR, LRESULT* pResult )
{
	int nSel = GetCurSel();
	UpdateTableDisplay(nSel);
}

void CARCportTabCtrl::UpdateTableDisplay( int nSel )
{
	if (nSel == 0)
	{
		m_wndScheduleListCtrl.ShowWindow(TRUE);
		m_wndRosterListCtrl.ShowWindow(FALSE);
	}
	else
	{
		m_wndScheduleListCtrl.ShowWindow(FALSE);
		m_wndRosterListCtrl.ShowWindow(TRUE);
	}
}

void CARCportTabCtrl::OnSize( UINT nType, int cx, int cy )
{
	CTabCtrl::OnSize(nType, cx, cy);

	if (m_wndRosterListCtrl.GetSafeHwnd() == NULL)
		return;

	m_wndScheduleListCtrl.SetWindowPos(NULL,0,25,cx,cy - 25,NULL);
	m_wndRosterListCtrl.SetWindowPos(NULL,0,25,cx,cy - 25,NULL);
}
