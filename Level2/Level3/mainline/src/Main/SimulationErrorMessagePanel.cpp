...
BEGIN_MESSAGE_MAP(SimulationErrorMessagePanel, CTabCtrl)
	ON_WM_SIZE()
	...
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST_ONBOARD, OnColumnclickListErrorMsgOnBoard)
	ON_NOTIFY_REFLECT(TCN_SELCHANGE,OnSelChangeTabList)
END_MESSAGE_MAP()
										 ...
										 const char* const colLabels[], const int nFormats[], const int nDefWidths[], const EDataType colDataType[], int nColNum)
{
	...
	GetWindowRect(&rect);
	ScreenToClient(&rect);
	static const CRect rcListCtrl(0,25,rect.right,rect.bottom);
	...
	ctrl.SetFont(GetParent()->GetFont());
	...
	if (InsertItem(nTabIndex,strTabName) == -1)
	{
		return FALSE;
	}
...
BOOL SimulationErrorMessagePanel::Create(CWnd* pWnd, const CRect& rect)
{
// 	if (!CXTTabCtrl::Create(WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_TABSTOP, tabRC, pWnd, IDC_SIMERRMSG_PANEL))
// 		return FALSE ;
	if (!CTabCtrl::Create(WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_CLIPCHILDREN|WS_TABSTOP,rect,pWnd,IDC_SIMERRMSG_PANEL))
		return FALSE ;
...
void SimulationErrorMessagePanel::OnSelChangeTabList( NMHDR* pNMHDR, LRESULT* pResult )
{
	int nSel = GetCurSel();
	UpdateTableDisplay(nSel);
}

void SimulationErrorMessagePanel::UpdateTableDisplay( int nSel )
{
	switch (nSel)
	{
	case 0:
		{
			m_wndAirsideListCtrl.ShowWindow(TRUE);
			m_wndLandsideListCtrl.ShowWindow(FALSE);
			m_wndTerminalListCtrl.ShowWindow(FALSE);
			m_wndOnBoardListCtrl.ShowWindow(FALSE);
			m_wndCargoListCtrl.ShowWindow(FALSE);
			m_wndEnvironmentListCtrl.ShowWindow(FALSE);
			m_wndFinancialListCtrl.ShowWindow(FALSE);
		}
		break;
	case 1:
		{
			m_wndAirsideListCtrl.ShowWindow(FALSE);
			m_wndLandsideListCtrl.ShowWindow(TRUE);
			m_wndTerminalListCtrl.ShowWindow(FALSE);
			m_wndOnBoardListCtrl.ShowWindow(FALSE);
			m_wndCargoListCtrl.ShowWindow(FALSE);
			m_wndEnvironmentListCtrl.ShowWindow(FALSE);
	...
	case 2:
		{
			m_wndAirsideListCtrl.ShowWindow(FALSE);
			m_wndLandsideListCtrl.ShowWindow(FALSE);
			m_wndTerminalListCtrl.ShowWindow(TRUE);
			m_wndOnBoardListCtrl.ShowWindow(FALSE);
			m_wndCargoListCtrl.ShowWindow(FALSE);
			m_wndEnvironmentListCtrl.ShowWindow(FALSE);
			m_wndFinancialListCtrl.ShowWindow(FALSE);
	...
	case 3:
		{
			...
			m_wndLandsideListCtrl.ShowWindow(FALSE);
			m_wndTerminalListCtrl.ShowWindow(FALSE);
			m_wndOnBoardListCtrl.ShowWindow(TRUE);
			m_wndCargoListCtrl.ShowWindow(FALSE);
			m_wndEnvironmentListCtrl.ShowWindow(FALSE);
			m_wndFinancialListCtrl.ShowWindow(FALSE);
		}
	...
	case 4:
		{
			...
			m_wndTerminalListCtrl.ShowWindow(FALSE);
			m_wndOnBoardListCtrl.ShowWindow(FALSE);
			m_wndCargoListCtrl.ShowWindow(TRUE);
			m_wndEnvironmentListCtrl.ShowWindow(FALSE);
			m_wndFinancialListCtrl.ShowWindow(FALSE);
		}
		break;
	case 5:
		{
			...
			m_wndOnBoardListCtrl.ShowWindow(FALSE);
			m_wndCargoListCtrl.ShowWindow(FALSE);
			m_wndEnvironmentListCtrl.ShowWindow(TRUE);
			m_wndFinancialListCtrl.ShowWindow(FALSE);
		}
		break;
	case 6:
		{
			...
			m_wndCargoListCtrl.ShowWindow(FALSE);
			m_wndEnvironmentListCtrl.ShowWindow(FALSE);
			m_wndFinancialListCtrl.ShowWindow(TRUE);
		}
		break;
	default:
		break;
...
void SimulationErrorMessagePanel::OnSize( UINT nType, int cx, int cy )
{
	CTabCtrl::OnSize(nType, cx, cy);

	if (m_wndTerminalListCtrl.GetSafeHwnd() == NULL)
		return;
	
	m_wndAirsideListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
	m_wndLandsideListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
	m_wndTerminalListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
	m_wndOnBoardListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
	m_wndCargoListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
	m_wndEnvironmentListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
	m_wndFinancialListCtrl.SetWindowPos(NULL,0,25,cx,cy - 20,NULL);
}
