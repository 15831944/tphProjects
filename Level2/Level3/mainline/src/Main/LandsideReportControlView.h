...
// LandsideReportControlView form view

class LandsideReportControlView : public CFormView
{
	DECLARE_DYNCREATE(LandsideReportControlView)

protected:
	LandsideReportControlView();           // protected constructor used by dynamic creation
	virtual ~LandsideReportControlView();

public:
	enum { IDD = IDD_DIALOG_REPORT_SPECS_LANDSIDE };
#ifdef _DEBUG
...
protected:
	LandsideBaseParam *m_pParameter;
...
protected:
	...
	LandsideReportTreeBasePerformer* GetTreePerformer();
protected:
	//the tree control's performer, it can be different for different report type	
	LandsideReportTreeBasePerformer *m_pTreePerformer;
