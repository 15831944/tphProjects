...
BEGIN_MESSAGE_MAP(CToolBarXP, CToolBar)
	...
	ON_CBN_SELCHANGE(ID_COMBOBOX,OnSelChangeZoomComboBox)
	ON_WM_CREATE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CToolBarXP::OnButtonDropDown (NMHDR* lpnmhdr, LRESULT* plResult)
{
    ...
    *plResult = TBDDRET_DEFAULT;
	...
	{
		if ( !GetParent()->SendMessage (WM_COMMAND, DROPDOWN(lpnmtb->iItem)) )
		{
			*plResult = TBDDRET_TREATPRESSED;
		}
	}
...
BOOL CToolBarXP::CreateComboBox()
{
	SetButtonInfo(6,ID_COMBOBOX,TBBS_SEPARATOR,160);
	CRect rect;
	GetItemRect(6,&rect);
	rect.bottom=rect.top+200;
	m_cmbScale.Create(WS_CHILD|WS_VISIBLE|WS_VSCROLL|CBS_DROPDOWNLIST,rect,this,ID_COMBOBOX);
    ...
    for(int i=0;i<nCount;i++)
    {
	  DefWindowProc(TB_GETITEMRECT,(WPARAM) i,(LPARAM) (LPRECT)&rc);
	  DefWindowProc(TB_GETBUTTON,(WPARAM) i,(LPARAM) (LPTBBUTTON)&tbb);
	  if(tbb.idCommand!=0)
	  {
		m_toolTipCtrl.DelTool(&(GetToolBarCtrl()), tbb.idCommand);
 		m_toolTipCtrl.AddTool(&(GetToolBarCtrl()), tbb.idCommand, &rc, tbb.idCommand);
	  }
    }
...
void CToolBarXP::OnSelChangeZoomComboBox()
{
	int nIndex=m_cmbScale.GetCurSel();
	DWORD dwScale=m_cmbScale.GetItemData(nIndex);
	
	GetParent()->SendMessage(UM_SELCHAGNE_SCALE_COMBOBOX,dwScale);
}

int CToolBarXP::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	GetToolBarCtrl().SetToolTips(&m_toolTipCtrl);
	return 0;
}

void CToolBarXP::ShowBtn(UINT nID, BOOL bShow)
{
...
int CToolBarXP::AddItem2CMBWithAI(int nTickWidth,BOOL bSelecting)
{
	m_cmbScale.ResetContent();
	int nIndex,nIndex100;
	nIndex=m_cmbScale.AddString("48 Hours");
	m_cmbScale.SetItemData(nIndex,48*60*60);

	nIndex=m_cmbScale.AddString("24 Hours");
	m_cmbScale.SetItemData(nIndex,24*60*60);
	nIndex=m_cmbScale.AddString("12 Hours");
	m_cmbScale.SetItemData(nIndex,12*60*60);
	
	nIndex=m_cmbScale.AddString("6 Hours");
	m_cmbScale.SetItemData(nIndex,6*6060);
	nIndex=m_cmbScale.AddString("3 Hours");
	m_cmbScale.SetItemData(nIndex,3*60*60);
	nIndex=m_cmbScale.AddString("1 Hours");
	m_cmbScale.SetItemData(nIndex,1*60*60);
	nIndex=m_cmbScale.AddString("40 Mins");
	m_cmbScale.SetItemData(nIndex,40*60);
	nIndex100=nIndex=m_cmbScale.AddString("30 Mins");
	m_cmbScale.SetItemData(nIndex,30*60);
	nIndex=m_cmbScale.AddString("20 Mins");
	m_cmbScale.SetItemData(nIndex,20*60);
	nIndex=m_cmbScale.AddString("10 Mins");
	m_cmbScale.SetItemData(nIndex,10*60);
	nIndex=m_cmbScale.AddString("5 Mins");
	m_cmbScale.SetItemData(nIndex,5*60);
	if(bSelecting)
		m_cmbScale.SetCurSel(nIndex100);
	
	return nIndex100;
}

void CToolBarXP::SetSelect(DWORD dItemData)
{
	int nCount=m_cmbScale.GetCount();
	for(int i=0;i<nCount;i++)
	{
		if(m_cmbScale.GetItemData(i)==dItemData)
		{
			m_cmbScale.SetCurSel(i);
			break;
		}
	}
}
