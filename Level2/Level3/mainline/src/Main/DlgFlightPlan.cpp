...
void CDlgFlightPlan::OnNewFlightPlan()
{
	...
	m_lstFltType.ResetContent();
	for (int i = 0; i < m_flightPlans.GetCount(); i++)
	{
		FlightPlanBase* pFlightPlan = m_flightPlans.GetFlightPlan(i);
		int nIndex = m_flightPlans.GetIndex(pFlightPlan);
		...
		CString fltcnststr;
		fltCnst.screenPrint(fltcnststr);
		//CString strFltType = pFlightPlan->GetFlightType();
		m_lstFltType.InsertString(nIndex,fltcnststr);
	}
	m_pCurFltPlan = pGenericProfile;
	int nCurIndex = m_flightPlans.GetIndex(pGenericProfile);
	ASSERT(nCurIndex != LB_ERR);
	m_lstFltType.SetCurSel(nCurIndex);
	UpdateCurrentPlanContent();
// 	if (nIndex != LB_ERR)
// 	{
// 
