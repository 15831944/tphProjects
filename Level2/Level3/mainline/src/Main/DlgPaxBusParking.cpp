...
CDlgPaxBusParking::CDlgPaxBusParking(int nProjID, InputTerminal* _pInTerm, CWnd* pParent /*=NULL*/)
	...
	,n_selectedItem(0)
{
	m_pPaxBusParkingList = new CPaxBusParkingList;
	m_nFlag = 0;
}
...
BOOL CDlgPaxBusParking::OnInitDialog()
{
	...
	InitToolBar();	
	InitListCtrl();
	m_pPaxBusParkingList->ReadData(m_nProjID);
...
{
	size_t nCount = m_pPaxBusParkingList->GetElementCount();

	CString strValue;
	for (size_t i = 0; i < nCount; i++)
	{
		CPaxBusParking* pItem = m_pPaxBusParkingList->GetItem(i);

		m_wndListCtrl.InsertItem((int)i, pItem->GetGate());
...
void CDlgPaxBusParking::OnNewItem()
{
	...
	if(IDOK == dlg.DoModal())
	{
		CPaxBusParking* pNewItem = new CPaxBusParking;
...
//	int nCurSubSel = dispinfo->item.iSubItem;	
	CPaxBusParking* pItem = (CPaxBusParking*)m_wndListCtrl.GetItemData(nCurSel);
...
void CDlgPaxBusParking::OnOK()
{
	...
	try
	{
		...
		m_pPaxBusParkingList->SaveData(m_nProjID);
...
{
	int nItem = GetListCtrlSelectedItem();
	CPaxBusParking* pItem = (CPaxBusParking*)m_wndListCtrl.GetItemData(nItem);
	pItem->SetPath(path);
	if(path.getCount() > 2 )
	{
...
void CDlgPaxBusParking::OnBnClickedButtonPaste()
{
	...
	if(n_selectedItem != -1)
	{
       ...
       CPaxBusParking* copenew = new CPaxBusParking;
       CPaxBusParking* selected = (CPaxBusParking*)m_wndListCtrl.GetItemData(n_selectedItem);
