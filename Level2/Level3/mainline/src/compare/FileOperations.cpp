...
void CFileOperation::DoFolderCopy(CString sSourceFolder, CString sDestFolder,void (CALLBACK* _ShowCopyInfo)(int, LPCTSTR),bool bDelteAfterCopy)
{
	...
	while (bRes)
	{
		...
		if (ff.IsDirectory()) // source is a folder
		{
			...
			DoFolderCopy(sPathSource, sPathDest,_ShowCopyInfo, bDelteAfterCopy);
		}
		else // source is a file
		{
			CString sNewFileName = sDestFolder + ff.GetFileName();
			DoFileCopy(ff.GetFilePath(), sNewFileName, _ShowCopyInfo,bDelteAfterCopy);
		}
	}
	ff.Close();
}
...
void CFileOperation::DoCopy(CString sSource, CString sDest,void (CALLBACK* _ShowCopyInfo)(int, LPCTSTR),bool bDelteAfterCopy)
{
	...
	if (CheckPath(sSource) == PATH_IS_FOLDER && CheckPath(sDest) == PATH_IS_FOLDER) 
	{
		...
		ff.Close();
		DoFolderCopy(sSource, sDest,_ShowCopyInfo,bDelteAfterCopy);
	}
	// file to file
	if (CheckPath(sSource) == PATH_IS_FILE && CheckPath(sDest) == PATH_IS_FILE) 
	{
		DoFileCopy(sSource, sDest,_ShowCopyInfo);
	}
	// file to folder
...
bool CFileOperation::Copy(CString sSource, CString sDest,void (CALLBACK* _ShowCopyInfo)(int, LPCTSTR))
{
	...
	try
	{
		DoCopy(sSource, sDest,_ShowCopyInfo);
		bRes = true;
	}
	catch(CFExeption* e)
	{
...
bool CFileOperation::Replace(CString sSource, CString sDest)
{
	...
	try
	{
		...
		DoCopy(sSource, sDest,NULL,true);
		DoDelete(sSource);
		m_bAskIfReadOnly = b;
		bRes = true;
	}
...
bool CFileOperation::NotCopyFolderOfSimResult(CString sSource, CString sDest,void (CALLBACK* _ShowCopyInfo)(int, LPCTSTR))
{
	...
	try
	{
		NotDoCopyFolderOfSimResult(sSource, sDest,_ShowCopyInfo);
		bRes = true;
	}
	catch(CFExeption* e)
	{
...
void CFileOperation::NotDoCopyFolderOfSimResult(CString sSource, CString sDest,void (CALLBACK* _ShowCopyInfo)(int, LPCTSTR), bool bDelteAfterCopy )
{
	...
	if (CheckPath(sSource) == PATH_IS_FOLDER && CheckPath(sDest) == PATH_IS_FOLDER) 
	{
		...
		ff.Close();
		NoCopyFolderOfSimResult(sSource, sDest,_ShowCopyInfo,bDelteAfterCopy);
	}
	// file to file
	if (CheckPath(sSource) == PATH_IS_FILE && CheckPath(sDest) == PATH_IS_FILE) 
	{
