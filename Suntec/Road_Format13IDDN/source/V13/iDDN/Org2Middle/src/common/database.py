...
class pg_client(object):
    ...
    def __init__(self, flag=1):
        self.log = common.common_log.instance().logger('DataBase')
        self.recordno = 0
        ...
        if flag != 1:  #  源数据库 和 目标数据库相同
            self.srv_path1 = self.srv_path2 = common.GetPath('db1')
        else:  #  源数据库 和 目标数据库可以不相同
            self.srv_path1 = common.GetPath('db1')
            self.srv_path2 = common.GetPath('db2')

        if self.srv_path1 == '' or self.srv_path2 == '':
            self.log.error("Doesn't exist host IP")
            common.proj_exit(1)

        self.SetSqlScriptPath(common.GetProjName())
    ...
    def SetSqlScriptPath(self, modulename):
        if modulename == 'proj_chn' :
            self.SqlScriprt = common.SqlScriprt_chn()
            self.SqlScriprt.init()
        elif modulename == 'proj_jpn':
            self.SqlScriprt = common.SqlScriprt_jpn()
            self.SqlScriprt.init()
        elif modulename == 'proj_rdf':
            self.SqlScriprt = common.SqlScriprt_rdf()
            self.SqlScriprt.init()
        elif modulename == 'proj_axf':
            self.SqlScriprt = common.SqlScriprt_axf()
            self.SqlScriprt.init()
        elif  modulename == 'proj_ta':
            self.SqlScriprt = common.SqlScriprt_ta()
            self.SqlScriprt.init()
        elif modulename == 'proj_jdb':
            self.SqlScriprt = common.SqlScriprt_jdb()
            self.SqlScriprt.init()
        elif modulename == 'proj_nostra':
            from common import sql
            self.SqlScriprt = sql.SqlScriprt_nostra()
            self.SqlScriprt.init()
        elif modulename == 'proj_mmi':
            from common.sql import SqlScriprt_mmi
            self.SqlScriprt = SqlScriprt_mmi()
            self.SqlScriprt.init()
        elif modulename == 'proj_msm':
            from common.sql import SqlScriprt_msm
            self.SqlScriprt = SqlScriprt_msm()
            self.SqlScriprt.init()
        else:
            self.SqlScriprt = common.SqlScriprt()
            self.SqlScriprt.init()
