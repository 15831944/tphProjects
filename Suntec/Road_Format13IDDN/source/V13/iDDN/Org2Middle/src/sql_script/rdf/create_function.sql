...
BEGIN
    ...
    FOR rec IN
    	...
    	from
    	(
    		select distinct linkid, cond_id
    		from
    		(
				-- regulation with condition
				select condition_id, cond_id, link_id as linkid
				from
				(
			        select a.condition_id, a.access_id, a.dt_id, b.link_id
			        from
    ...
    LOOP
		...
		while nIndex <= nCount LOOP
    		insert into regulation_relation_tbl
    					("regulation_id", "nodeid", "inlinkid", "outlinkid", "condtype", "cond_id")
    			VALUES	(cur_regulation_id, null, rec.linkid, null, 4, rec.array_condid[nIndex]);
    		nIndex := nIndex + 1;
    	END LOOP;
    END LOOP;
    return 1;
...
BEGIN
	...
	while iterator < valid_node_cnt loop
		select link_id into linkid
		from rdf_link 
		where ((ref_node_id = nodeid_list[iterator] and nonref_node_id = nodeid_list[iterator +1]) 
		OR (nonref_node_id = nodeid_list[iterator] and ref_node_id = nodeid_list[iterator +1]));
                                ...
                                for rec in 
                                        select e.nextroad,e.dir,e.nextnode
                                                ...
                                                (
                                                        ...
                                                        FROM (
                                                                SELECT link_id as routeid, s_node as start_node_id, e_node as end_node_id,one_way_code as oneway
                                                                FROM link_tbl
                                                        ) as c
                                                        where c.start_node_id = tmpLastNodeArray[tmpPathIndex] and oneway in (1,2) 
                                                        ...
                                                        where c.end_node_id = tmpLastNodeArray[tmpPathIndex] and oneway in (1,3)
                                                ) as e
                                loop
	...
	loop
                ...
                while i <= 5 and (rec.nodes)[i] is not null loop
                        ...
                        from(
                                ...
                                where zlevel = (rec.levels)[i] and ST_Distance(the_geom,(rec.nodes)[i],true) <= deata
                        )as a
                        group by id;
