...
BEGIN
    ...
    ELSEIF ft = 2 THEN
    	return 11;
    ELSEIF procstat in (8,9) or stubble = 1 THEN
    	return 9;
    ELSEIF privaterd != 0 THEN 
    	return 7;
    ELSEIF fow in (14, 15, 18, 19) or backrd in (1, 2, 3, 4) THEN
    	return 8;
...
CREATE OR REPLACE FUNCTION mid_cnv_disp_class(freeway smallint, frc smallint, feattyp smallint, fow smallint )
  RETURNS smallint
  LANGUAGE plpgsql AS
$$
BEGIN
...
-- admin
-------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION mid_Get_BigIDToInt(id bigint)
  RETURNS integer 
  LANGUAGE plpgsql
  AS
$$
DECLARE
BEGIN
	return 100000*cast(cast (id / 10000000000 as integer) % 1000 as integer) + cast(id % 100000 as integer);
END;
$$;
...
BEGIN
  ...
  select (case when max(gid) is null then 0 else max(gid) end)
	from temp_adminid_newandold
	into cur_regulation_id;
	---order0
	INSERT INTO temp_adminid_newandold(ID_old,level)
	SELECT ID_old,
				 level
	FROM
	(
  ...
  group by ID_old,level
  order by ID_old,level;
  ---update
  update temp_adminid_newandold set New_ID = mid_admin_SetNewID(ID_old,gid - cur_regulation_id,0) where level = 0;
  ...
  order by ID_old,level;
  ---update
  update temp_adminid_newandold set New_ID = mid_admin_SetNewID(ID_old,gid - cur_regulation_id,1) where level = 1;
  ...
  order by ID_old, level;
   ---update
   update temp_adminid_newandold set New_ID = mid_admin_SetNewID(ID_old,gid - cur_regulation_id,3) where level = 3;
...
BEGIN
  ...
  SELECT  order0_id, 
    ...
    group by order0_id, order0_code, name_id,country_code
    order by order0_id, order0_code, name_id,country_code;
...
BEGIN
	...
	for rec in
	(
		select id,order00 from org_a0 group by id,order00
	)
	loop
...
BEGIN
	...
	for countryidx in 1..countrycount loop
		if Countryarray[countryidx] = 'VNM' then
