...
class comp_link_dupli_or_circle(base.component_base.comp_base):

    def __init__(self):

        base.component_base.comp_base.__init__(self, 'Link_overlay_or_circle')
...
#            self.log.info('backuping %s...' % the_guide_table)
    ...
    def __dupli_link_dealing_link(self):
        ...
        sqlcmd = """
           insert into link_tbl
                    ...
                    (
                        select  link_id, tile_id, s_node, e_node, link_type, road_type, toll, speed_class, length, function_class,
                                    lane_dir, lane_num_s2e, lane_num_e2s, elevated, structure, tunnel, rail_cross, paved, uturn,
                                    speed_limit_s2e, speed_limit_e2s, speed_source_s2e, speed_source_e2s, width_s2e, width_e2s,
    ...
    def __dupli_link_dealing_node(self):
        ...
        sqlcmd = """
                insert into node_tbl(node_id,kind,light_flag,toll_flag,org_boundary_flag,tile_boundary_flag,mainnodeid,node_lid,node_name,the_geom)
                select node_id,kind,light_flag,toll_flag,org_boundary_flag,tile_boundary_flag,mainnodeid,array_to_string(array_agg(conne_link),'|') as node_lid,node_name,the_geom
                from (
                    select  a.node_id,
                        a.kind,
                        a.light_flag,
                        ...
                        a.the_geom
                    from (
                            select node_id,kind,light_flag,toll_flag,org_boundary_flag,tile_boundary_flag,mainnodeid,
                ...
                group by node_id,kind,light_flag,toll_flag,org_boundary_flag,tile_boundary_flag,mainnodeid,node_name,the_geom;
           """
        ...
        sqlcmd = """
                   insert into link_tbl
                    (
                        ...
                        one_way_code, one_way_condition, pass_code, pass_code_condition, road_name, road_number,
                        name_type, ownership, car_only, slope_code, slope_angle, disobey_flag, up_down_distinguish,
                        access, extend_flag, etc_only_flag, display_class, fazm, tazm, the_geom
                    )
                    (
                        select  b.link_id, b.tile_id, b.s_node, b.e_node, link_type, road_type, toll, speed_class, ST_Length_Spheroid(b.the_geom,'SPHEROID("WGS_84", 6378137, 298.257223563)') as length,
                                ...
                                mid_cal_zm(b.the_geom, 1) as fazm,
                                mid_cal_zm(b.the_geom, -1) as tazm,
    ...
    def __circle_link_dealing_node(self):
        ...
        sqlcmd = """
            ...
            select node_id,null as kind,null as light_flag,null as toll_flag,0 as mainnodeid,array_to_string(array_agg(link_id),'|'), null as node_name,the_geom from (
