...
class comp_link_jdb(base.component_base.comp_base):
    '''
    classdocs
    '''
    def __init__(self):
    ...
    def _Do(self):
        ...
        select h.tile_link_id as link_id              -- link_id           
            ...
            , rdb_cnv_link_type(linkclass_c) as link_type                 -- link_type
            , rdb_cnv_road_type(
            ...
            , rdb_cnv_function_class(
                    naviclass_c, widearea_f
            ) as function_class                                           -- function_class
            , mid_linkdir(oneway_c) as lane_dir                           -- lane dir
            , get_lanecount_downflow(oneway_c, lane_count)                -- lane_num_s2e
            , get_lanecount_upflow(oneway_c,lane_count)                   -- lane_num_e2s
            ...
            , get_width_downflow(oneway_c, rdb_linkwidth(roadwidth_c))    -- width_s2e
            , get_width_upflow(oneway_c, rdb_linkwidth(roadwidth_c))      -- width_e2s
            ...
            , case when d.road_code is not null then
                    mid_get_json_string_for_japan_name(rdb_cnv_name_suffix(d.name_kanji),d.name_yomi) 
                   when e.road_code is not null then
                    mid_get_json_string_for_japan_name(rdb_cnv_name_suffix(e.name_kanji),e.name_yomi)                    
                   else null
