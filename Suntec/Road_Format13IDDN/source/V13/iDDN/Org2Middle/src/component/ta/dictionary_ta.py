...
from component.multi_lang_name import TTS_TYPE_NOT_TTS
...
# comp_dictionary_ta
#==============================================================================
class comp_dictionary_ta(component.dictionary.comp_dictionary):
    '''字典(TomTom)
    '''
    ...
    def _get_shield_info(self,
        ...
        for shieldid, shieldnum, routenum, phoneme_info in \
            ...
            if not shieldid:  # shield id 无
                self.log.error("No new shield id. link_id=%s", link_id)
                continue
            if not shieldnum:  # shield number 无
                self.log.error("No new shield num. link_id=%s", link_id)
                continue
            if not routenum:  # route number 无
                self.log.error("No new route num. link_id=%s", link_id)
                continue
