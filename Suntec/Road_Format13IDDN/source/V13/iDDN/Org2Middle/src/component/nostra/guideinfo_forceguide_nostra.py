...
'''
import os
from component.guideinfo_forceguide import com_guideinfo_forceguide
import component
import psycopg2
...
class comp_guideinfo_forceguide_nostra(com_guideinfo_forceguide):
    def __init__(self):
        '''
        Constructor
        '''
        component.guideinfo_forceguide.com_guideinfo_forceguide.__init__(self)
...
#         self.pg.commit2()
        ...
        self._check_org_mid_num(forceguide_patch_full_path)
        return 0
    ...
    def _make_forceguide_tbl(self):
        ...
        '''
        sql_cmd = '''
            SELECT objectid, fromlinkid, tolinkid, midcount, midlinkid, guidetype,nodeid
    ...
    def _get_connectnode_by_links(self, inlink, outlink):
        sql_cmd = '''
            select a.start_node_id,a.end_node_id,b.routeid
            from
            (
            ...
            left join
            (        SELECT start_node_id, end_node_id,routeid
                    FROM temp_topo_link
                    where routeid = %s
            ) as b
        ...
        sqlcmd = """
            ...
            (
                select gid, link_array[1] as fromlinkid,
                    ...
                    guide_type as guidetype,nodeid
                from
                (
                    select gid, link_array,
                        array_upper(link_array,1) as link_cnt, guide_type,node2 as nodeid
                    from
                    (
                        select gid, node2, mid_make_link_list_by_nodes(array[node1,node2,node3,node4,node5]) as link_array,guide_type
        ...
        for c_row in c_rows:
            ...
            for i in xrange(lenth - 1):
                ...
                if f_node and t_node:
                    ...
                    if sub_linklib:
                        if i == 0:
                            sublinklist = sub_linklib.split(',')
                            inlinkid = sublinklist[len(sublinklist) - 1]
                            all_linklib = all_linklib + inlinkid + ','
                        else:
            ...
            if length >= 3:
                ...
                SET fromlinkid=%s, tolinkid=%s, midcount=%s,
                    midlinkid=%s
                 WHERE objectid = %s;
                '''.format(temp_tbl_name), (int(inlinkid), link_array[length - 2], length - 3,
