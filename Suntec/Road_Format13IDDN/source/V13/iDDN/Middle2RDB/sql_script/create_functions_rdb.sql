...
CREATE OR REPLACE FUNCTION rdb_convert_node_extendflag(light_flag boolean, dr_flag boolean, br_flag boolean, 
													  ...
													  cross_name_flag boolean, toll boolean,
...
BEGIN
	...
	if force_flag = TRUE THEN
		ExtendFlag := ExtendFlag | (1::smallint << 15);  -- Forceguide
	END IF;
...
CREATE OR REPLACE FUNCTION rdb_make_tile_links()
  RETURNS integer 
  LANGUAGE plpgsql VOLATILE
AS $$ 
DECLARE
...
BEGIN
	...
	for rec in select * from temp_link_tbl order by tile_id,common_main_link_attri,link_id
	...
	LOOP
		...
		while nIndex  <= nCount loop	
			...
			insert into temp_link_tbl_newid ("link_id", "s_node", "e_node", "tile_id", "common_main_link_attri")
				values (link_array[nIndex], start_node, end_node, rec.tile_id,rec.common_main_link_attri);
			nIndex := nIndex + 1;	
		end loop;
...
CREATE OR REPLACE FUNCTION rdb_cnv_path_extra_info(paved smallint, uturn smallint) 
  RETURNS smallint 
  LANGUAGE plpgsql VOLATILE
  AS $$ 
DECLARE
...
BEGIN
	for rec in
		select	link_id, region_tile_id, start_node_id, end_node_id, one_way, 
				toll, road_name, road_number, road_type, pdm_flag, path_extra_info
...
BEGIN
	if st_isempty(i_geom) or st_geometrytype(i_geom) in ('ST_MultiLineString', 'ST_LineString') then
		...
		(
			...
			where st_geometrytype(sub_geom) != 'ST_Polygon'
			order by index
		)as c
		into i_geom;
		return i_geom;
...
-- 		raise info '  ---1---Pnum :%',Pnum;
		...
		for i in 0..(Nnum - 1) loop
			...
			Fpoint := ST_Line_Locate_Point(geom, ST_pointN(geom,(i + 1)));
...
--  				raise info '  ---else---Pnum :%',Pnum;
				...
				if Fpoint < ST_Line_Locate_Point(geom, ST_pointN(geom,i)) then
					Fpoint := 1 - ST_Line_Locate_Point(st_reverse(geom), ST_pointN(geom,(i + 1)));
 					raise info 'reverse!';
				end if;
...
BEGIN
	...
	for link_num_idx in 1..link_num loop
		if node_type = 0 then
			...
			if (link_arrry[link_num_idx]::bit(32))::integer > 65535 then
