    ...
    def Do(self):
        ...
        sqlcmd = """
           INSERT INTO rdb_link(
                link_id, link_id_t, display_class, start_node_id, start_node_id_t, end_node_id, end_node_id_t,
                ...
                link_type, extend_flag, fazm_path, tazm_path, shield_flag, the_geom, the_geom_900913)
                (
                 SELECT link_id, link_id_t, display_class, s_node_id, start_node_id_t, end_node_id, end_node_id_t,
                      ...
                      extend_flag, fazm_path, tazm_path, shield_flag, the_geom, the_geom_900913
                 from (
        ...
        sqlcmd = """
            ...
            where
            rdb_link.link_id = A.link_id
            """
        rdb_log.log(self.ItemName, 'Now it is simplify shape points of rdb_link when the count > 1024...', 'info')
        if self.pg.execute2(sqlcmd) == -1:
            return -1
        else:
            self.pg.commit2()
...
class rdb_link_simplify_axf_china(ItemBase):
    ...
    def Do(self):
        ...
        sqlcmd = """
                INSERT INTO rdb_link(
                    link_id, link_id_t, display_class, start_node_id, start_node_id_t, 
                    ...
                    fnlink, fnlink_t, tnlink, tnlink_t, link_type, extend_flag, fazm_path, 
                    tazm_path, shield_flag, the_geom, the_geom_900913)
                (
                select  a.link_id, link_id_t, display_class, start_node_id, start_node_id_t, 
                        ...
                        tilt_flag, speed_regulation_flag, link_add_info_flag, fazm, tazm, 
                        fnlink, fnlink_t, tnlink, tnlink_t, link_type, extend_flag, fazm_path, 
                        tazm_path, shield_flag, b.the_geom, c.the_geom_900913
                from temp_simplify_rdb_link_backup as a
    ...
    def _updateShape(self):
        ...
        sqlcmd = """
                INSERT INTO rdb_link(
                    ...
                    tilt_flag, speed_regulation_flag, link_add_info_flag, fazm, tazm, 
                    fnlink, fnlink_t, tnlink, tnlink_t, link_type, extend_flag, fazm_path, 
                    tazm_path, shield_flag,
                    the_geom, the_geom_900913, the_geom_4096)
                (
                select  a.link_id, a.link_id_t, display_class, a.start_node_id, start_node_id_t, a.end_node_id, end_node_id_t,
...
class rdb_link_wellshape_handle_rdf(ItemBase):
    def __init__(self):
        ItemBase.__init__(self, 'Wellshape_Handle')
