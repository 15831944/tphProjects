...
class rdb_pg(object):
    ...
    def __init__(self, flag = 1):
        ...
        if flag != 1:      #  源数据库 和 目标数据库相同
            self.srv_path1 = self.srv_path2 = rdb_common.GetPath('db1')
        else:    #  源数据库 和 目标数据库可以不相同
            self.srv_path1 = rdb_common.GetPath('db1')
            self.srv_path2 = rdb_common.GetPath('db2')
            
        if self.srv_path1 == '' or self.srv_path2 == '':
            rdb_log.log('DataBase', "Doesn't exist host IP", 'error')
            raise Exception, "host IP doesn't exist."
            
        self.SqlScriprt = rdb_sql.rdb_SqlScriprt_ipc()
