...
class theChecker(object):
    ...
    def _CheckIfAllPatternPicsExsit(self, thePatternPicDir):
        ...
        totalPatternCount = len(totalPicList)
        ...
        for row in rows:
            ...
            for onePic in totalPicList:
                if(onePic.find(patternno) > -1):
                    ...
                    break
            if(bFound == True):
                print '''pattern picture found: %s''' % patternno
            else:
        ...
        for row in rows:
            ...
            for onePic in totalPicList:
                if(onePic.find(arrowno) > -1):
                    ...
                    break
            if(bFound == True):
                print '''arrow picture found: %s''' % arrowno
            else:
                print '''error, arrow picture cannot be found, nodeid %s, inlinkid %s, 
                outlinkid %s, patternno %s''' % (nodeid, inlinkid, outlinkid, arrowno)
...
if __name__ == '__main__':
    ...
    aChecker._CheckIfAllPatternPicsExsit("C:\\My\\20150504-0511_ni_spotguide\\17CY Sample data_2\\data\\GraphAndVoice")
    aChecker._CheckIfAllArrowPicsExsit("C:\\My\\20150504-0511_ni_spotguide\\17CY Sample data_2\\data\\GraphAndVoice")
