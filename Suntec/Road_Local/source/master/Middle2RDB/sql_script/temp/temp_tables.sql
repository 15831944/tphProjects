...
(
	...
	(
		...
		-- regulation node of turn restriction
		union
		select  distinct 
				case 
				when snode_array[1] in (snode_array[2], enode_array[2]) then snode_array[1]
				else enode_array[1]
				end
		from
		(
			...
			from
			(
				select	a.record_no, a.seq,
						(case when b.link_id is not null then b.link_id else 0 end) as link_id,
						b.start_node_id, 
						b.end_node_id
				from
				(
					select	record_no, link_array, generate_series(1, link_num) as seq
					from
					(
						...
						where	(link_num > 1)
								and
								not (link_num = 2 and first_link_id = last_link_id)
					)as a
				)as a
				left join temp_region_links as b
				on a.link_array[a.seq] = b.link_id
				order by a.record_no, a.seq
			)as m
			group by record_no
			having not (0 = ANY(array_agg(link_id)))
		)as n
	...
	(
		...
		(
			...
			(
				...
				where a.link_num = 1 and b.record_no is null
			)as t1
		)as t2
	)as t3
);
...
as
(
	select org_link_id,target_link_id as rdb_link_id
		,case when s_fraction > e_fraction then e_fraction
			else s_fraction
		end as s_fraction
		,case when s_fraction > e_fraction then s_fraction
			else e_fraction
		end as e_fraction
		,flag
...
(
	select network_id::bigint as link_id
		...
		,round((meters / 1000 / spweek) * 3600) as average_time
		,case when spfreeflow is not null then true
			else false
		end as profile_flag
		,meters
	...
	from (
		select a.*
			,ST_Length_Spheroid(b.the_geom,'SPHEROID("WGS_84", 6378137, 298.257223563)') as meters 
		from org_hsnp a
		left join org_nw b
...
create table temp_forecast_link_rdb_id 
as
(
...
as
(
	select distinct time_slot from temp_forecast_time_org
	order by time_slot
);
