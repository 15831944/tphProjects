...
as
(
	select distinct in_link_id,in_angle,in_geom,
		out_link_id,out_geom,out_angle,cnt_link_id,angle,left_angle
	from (
		select m.*
		from (
			select * from (
				select in_link_id,in_angle,in_geom,
					...
					case when left_angle >= 360 then left_angle - 360
						 when left_angle < 0 then left_angle + 360 
						 else left_angle
					end as left_angle
				from (
					select in_link_id,in_angle,in_geom,
						...
						(in_angle - mid_angle) as left_angle
					from (
						select a.link_id as in_link_id,
							...
							case when a.e_node = c.s_node then rdb_cal_zm_lane(c.the_geom, 1) 
								when a.e_node = c.e_node then rdb_cal_zm_lane(c.the_geom, -1) 
							end as mid_angle
						from (
						...
						left join (
							select link_id,the_geom,s_node,e_node,road_type,link_type
							from link_tbl
							where link_type != 2 and one_way_code in (1,2)
						) c
						on a.e_node in (c.s_node)		
		...
		or mid_cnv_roadname_simplify(n1.name) 
		   is distinct from 
		   mid_cnv_roadname_simplify(n2.name)
	) q
); 
...
(
	...
	and not (folder = 'NPM' and (name like '{ME19%' or name like '{ME28%'))
);
