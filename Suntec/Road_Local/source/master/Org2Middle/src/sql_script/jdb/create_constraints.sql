...
CREATE INDEX temp_inode_geom_idx
   ON temp_inode
   USING gist (the_geom);
   
CREATE INDEX t_logomark_4326_the_geom_idx
  ON t_logomark_4326
  USING gist
  (the_geom);

CREATE INDEX temp_building_mapping_node_the_geom_circ_buffer_idx
  ON temp_building_mapping_node
  USING gist
  (the_geom_circ_buffer);
CREATE INDEX building_structure_the_geom_idx
  ON building_structure
  USING gist
  (the_geom);
CREATE INDEX link_tbl_road_type_idx
  ON link_tbl
  USING btree
  (road_type);