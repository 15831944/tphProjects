...
CREATE OR REPLACE FUNCTION mid_get_angle_sequence_temp(fstart_azm int, fend_azm int)
	RETURNS smallint
	LANGUAGE PLPGSQL
AS $$
DECLARE
...
BEGIN	
	...
	WHILE tmpPathIndex <= tmpPathCount LOOP
		...
		for rec in
			select	(case when s_node = tmpLastNodeArray[tmpPathIndex] then e_node else s_node end) as nextnode, 
					link_id as nextlink, 
					link_type, road_type, function_class,one_way_code,
					case when s_node=tmpLastNodeArray[tmpPathIndex] then mid_cal_zm(the_geom, 1) 
					     when e_node=tmpLastNodeArray[tmpPathIndex] then mid_cal_zm(the_geom, -1)
					     end as azm 
			from link_tbl
			where	(
                (
		...
		loop
			...
			if tmpPath is not null then
				...
				select case when s_node = tmpLastNodeArray[tmpPathIndex] then mid_cal_zm(the_geom, 1)
							when e_node = tmpLastNodeArray[tmpPathIndex] then mid_cal_zm(the_geom, -1)
							end into azm
				from link_tbl
				where link_id=(tmpPatharr[array_upper(tmpPatharr,1)]::bigint);
				-- raise info 'rec.azm= % ,azm= %, angle= %',rec.azm,azm,mid_get_angle_sequence_temp(rec.azm,azm);
			end if;  
			...
			if not (
				    ...
				    )  or mid_get_angle_sequence_temp(rec.azm,azm)>=130 then
            ...
            if tmpPath is not null then
                ...
                if rec.road_type in (0,1) and rec.link_type in (1,2) and rec.one_way_code not in (1,4) then
                   LinkType := 3;
                else 
                   LinkType := 5;
                end if;
            ...
            nCount		:= array_upper(link_array, 1);
            if LinkType = 3 then
                insert into temp_jct_link_paths(snode,path,elink,direction)
                   values(nHwyNode,link_array,link_array[nCount],dir);
            end if;
...
CREATE OR REPLACE FUNCTION mid_find_sapa_links_rev2_0115( link bigint, max_num integer,dir integer)
	RETURNS integer
        LANGUAGE plpgsql
	AS $$
	DECLARE        	
...
BEGIN
	...
	while link_num < max_num loop
      ...
      for i in 1..leng loop
          ...
          hw_jct_ic_flag = 0;
          temp_link_array = array[]::bigint[];
          for rec in 
              select link_id,link_type,road_type,one_way_code
              from link_tbl as a
          ...
          loop
              if rec.link_id is not null then
                  ...
                  if not (rec.link_type in (3,5,7) or ( rec.link_type in (1,2) and rec.road_type in (0,1) )) then
                      hw_jct_ic_flag = 1;
                     
                  end if;
                  
                  if not (rec.link_id = any(link_searched)) and rec.link_type not in (3,5,7) and rec.one_way_code <> 4 then
                      --link_arrayC := array_append(link_arrayC, rec.link_id);
                      temp_link_array := array_append(temp_link_array, rec.link_id);
                  end if;
              end if;
          end loop;
          
          if hw_jct_ic_flag = 1 then
               link_arrayC = link_arrayC || temp_link_array;
          end if;
	...
	while link_num < max_num loop
      ...
      for i in 1..leng loop
          ...
          loop
              if rec.link_id is not null then
                  raise info 'link_searched = %',link_searched;
                  if rec.link_type in (1,2) and rec.road_type in (0,1) then
                      hw_flag = 1;
                      exit;
                  end if; 
                  if not (rec.link_type in (3,5,7) or ( rec.link_type in (1,2) and rec.road_type in (0,1) )) then
          ...
          if jct_ic_flag = 0 and hw_flag = 0 then
               link_arrayC = link_arrayC || temp_link_array;
          end if;
...
CREATE OR REPLACE FUNCTION mid_change_sapa_bak(max_num integer)
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION mid_change_sapa(max_num integer,stat_sapa_max_num integer)
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	loop
		if rec.one_way_code = 1 then
			perform mid_find_sapa_links_rev3(rec.link_id::bigint, 2, max_num, stat_sapa_max_num);
			perform mid_find_sapa_links_rev3(rec.link_id::bigint, 3, max_num, stat_sapa_max_num);
			
		end if;

		if rec.one_way_code in (2,3) then
			perform mid_find_sapa_links_rev3(rec.link_id::bigint, rec.one_way_code, max_num, stat_sapa_max_num);
...
CREATE OR REPLACE FUNCTION  mid_find_jct_link_paths_bak1119(link bigint, dir integer,len integer)
	RETURNS smallint
        LANGUAGE plpgsql
	AS $$
	DECLARE 
		...
		temp_length	integer;
		temp_highway_link_array bigint[];
...
BEGIN
    ...
    while i <= array_length(link_path_array,1) loop
        ...
        if slink = link and direction = 1 then
            direction = dir + 1;
        end if;
        if direction = 4 then
           exit;
        ...
        if link_type1 in (1,2) and road_type1 in (0,1) or link_type1 = 3  then
              ...
              if flag = 1 and link_type1 in (1,2) then
                  insert into temp_suspect_jct_paths_link(link_path,direction) 
                        --values(link_path_array[i]);
                        values((string_to_array(link_path_array[i],'|'))::bigint[],dir );	
              end if;
              ...
              highway_count := 0;
              temp_link_array := array[]::bigint[];
              temp_highway_link_array := array[]::bigint[];
              for rec in 
                  ...
                  where (	  ( (a.s_node in (snode,enode) and a.one_way_code in(1,2))
                        ...
                        and 
                       ( a.link_type in (3,5,7) or (a.link_type in (1,2) and a.road_type in (0,1) and a.one_way_code not in (1,4)))
	
              loop
                  if rec.link_id is not null then
                        raise info 'rec.link_id = %, rec.link_type = %',rec.link_id, rec.link_type;
                        ...
                        if rec.link_id = any(link_array) then
                            ...
                            temp_link_array := array_append(temp_link_array,rec.link_id);
                            if rec.link_type in (1,2) then
                                temp_highway_link_array := array_append(temp_highway_link_array,rec.link_id);
                            end if;
             ...
             raise info 'temp_link_array = %',temp_link_array;
              raise info 'temp_highway_link_array = %',temp_highway_link_array;
             
             
             if highway_count > 0 and general_count > 0 then
                  temp_length := array_length(temp_highway_link_array,1);
                  if temp_length is not null then
                      for j in 1..temp_length loop
                          link_path_array := array_append(link_path_array,(link_path_array[i] || '|' || ((temp_highway_link_array[j])::varchar))::varchar);  
                     
                      end loop;
                  end if;
             else
                temp_length := array_length(temp_link_array,1);
                if temp_length is not null then
                    for j in 1..temp_length loop
                        link_path_array := array_append(link_path_array,(link_path_array[i] || '|' || ((temp_link_array[j])::varchar))::varchar);  
...
CREATE OR REPLACE FUNCTION  mid_find_jct_link_paths_bak0114(link bigint, dir integer,len integer)
	RETURNS smallint
        LANGUAGE plpgsql
	AS $$
	DECLARE 
    ...
    while i <= array_length(link_path_array,1) loop
        ...
        if slink = link then
              ...
              for rec in 
                select link_id
                from link_tbl as a
                where (enode in (a.s_node,a.e_node) and dir = 1
                      or 
                        snode in (a.s_node,a.e_node) and dir = 2
                      )
                    and a.link_type in (1,2) and a.road_type in (0,1) and a.one_way_code not in (1,4)
                loop
                    if rec.link_id is not null then
                      flag_hw_connect := 1;
                      raise info 'connect to highway!';
                    end if;

                end loop;
                raise info 'first link = %, enode = %',slink,enode;

                if flag_hw_connect = 1  then
                    i = i + 1;
                    continue;
                end if;
        end if;
...
BEGIN
   ...
   -- flag := 0;
    while i <= array_length(link_path_array,1) loop
        link_array := string_to_array(link_path_array[i],'|');
        if array_length(link_array,1) > 1 then
            for rec in
                select (case when a.s_node in (b.s_node,b.e_node) then a.s_node
                              else a.e_node end ) as attention_node
                from link_tbl as a
                join link_tbl as b
                on a.link_id = link_array[array_upper(link_array,1)] and b.link_id = link_array[array_upper(link_array,1) - 1]
            loop
                if rec.attention_node is not null then
                    attention_node = rec.attention_node;
                end if;
            end loop;
         else
            attention_node = 0;
        ...
        if link_type1 in (1,2) and road_type1 in (0,1) or link_type1 = 3  then
              ...
              for rec in 
                  select  link_id,link_type
                  from link_tbl as a
                  where (	  ( (a.s_node in (snode,enode) and a.one_way_code in(1,2)) and a.s_node <> attention_node
                              or
                                 (a.e_node in (snode,enode) and a.one_way_code in(1,3)) and a.e_node <> attention_node
                               )
                              and 
                              direction = 1 and dir = 1
                              
                          or
                            ( (a.e_node in (snode,enode) and a.one_way_code in(1,2)) and a.e_node <> attention_node
                              or
                                 (a.s_node in (snode,enode) and a.one_way_code in(1,3)) and a.s_node <> attention_node
                               )
                              and 
                              direction = 1 and dir = 2
                          ...
                          or
                              (
                                (a.s_node = enode  and a.one_way_code in(1,2)) and a.s_node <> attention_node
                               or 
                                (a.e_node = enode  and a.one_way_code in (1,3)) and a.e_node <> attention_node
                              )
                              and
                              direction <> 1 and dir = 1
                          ...
                          or
                              (
                                (a.e_node = snode  and a.one_way_code in(1,2)) and a.e_node <> attention_node
                               or 
                                (a.s_node = snode  and a.one_way_code in (1,3)) and a.s_node <> attention_node
                              )
                              and
                              direction <> 1 and dir = 2
                        )
...
CREATE OR REPLACE FUNCTION mid_calc_large_angle_path(path bigint[],angle double precision)
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION mid_calc_the_largest_angle_path(path bigint[],angle double precision)
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
	...
	rec record;
	steering_angle double precision;
	temp_angle	double precision;
BEGIN
	if array_length(path,1) is null or array_length(path,1) < 2 then
		return 0;
	end if;
	...
	loop
		for rec in 
			select distinct a.link_id as link1, b.link_id as link2,
				(case when a.s_node in (b.s_node,b.e_node) then a.s_node
				      when a.e_node in (b.s_node,b.e_node) then a.e_node end ) as join_node
			from link_tbl as a
			join link_tbl as b
			on a.link_id = path[i] and b.link_id = path[i+1]
		loop
			select mid_calc_jct_steering_angle(rec.link1,rec.link2,rec.join_node) into temp_angle;
			if temp_angle < steering_angle then
				steering_angle = temp_angle;
			end if;
...
CREATE OR REPLACE FUNCTION mid_filter_the_large_angle_jct2(path bigint[],angle double precision)
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	i = 1;
	length := array_length(path,1);
	while i <= array_length(path,1)
	loop
		
		link := path[i];
		for rec in 
			select distinct b.link_id, c.link_id as other_link,c.link_type ,
				(case when b.s_node in (c.s_node,c.e_node) then b.s_node
				      when b.e_node in (c.s_node,c.e_node) then b.e_node end ) as join_node
			from
			(
		...
		loop
          ...
          if (not rec.join_node = any(join_node_array) ) or array_length(join_node_array,1) is null then
              ...
              if rec.link_type not in (3,5,7)  and i not in (1,length) then
		raise info 'jct path connect to general road,link_type = %',rec.link_type;
		return 1;
              end if;
           end if;
			end if;
...
CREATE OR REPLACE FUNCTION mid_find_ic_surround_by_sapa(link bigint, dir integer)
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	while i <= array_length(ic_path_array,1) loop
		...
		if link_type1  = 7  then
			len := array_length(link_array,1);
			if len > 2 then
				raise info 'insert into----';
				insert into temp_ic_surround_by_sapa_link(links)
					values(link_array);
			end if;
			...
			temp_ic_path_array := (array[])::bigint[];
		      ...
		      loop
			  if rec.link_id is not null then
				if rec.link_type not in (5,7) then
            raise info 'the ic path connect to general road';
            flag := 1;
            exit;
				end if;
...
CREATE OR REPLACE FUNCTION mid_find_ic_surround_by_sapa_exec_bak()
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	for rec in 
      ...
      from
      (
          select d.link_id,d.one_way_code,array_agg(d.link_type) as link_type_array
          from
          (

              select distinct b.link_id, b.one_way_code, c.link_type
              from
              (
                  select * from link_tbl as a
                  where a.link_type = 5
          ...
          group by d.link_id, d.one_way_code
      ) as e
      where link_type_array = (array[7])::smallint[]
	loop
      if rec is not null then
          if rec.one_way_code = 1 then
              perform mid_find_ic_surround_by_sapa(rec.link_id, 2);
              perform mid_find_ic_surround_by_sapa(rec.link_id, 3);
          end if;
          if rec.one_way_code in (2,3) then
              perform mid_find_ic_surround_by_sapa(rec.link_id, rec.one_way_code);
          end if;
...
CREATE OR REPLACE FUNCTION mid_find_ic_surround_by_sapa_exec()
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	loop
      if rec is not null then
          if rec.one_way_code = 1 then
              perform mid_find_ic_surround_by_sapa_rec2(rec.link_id,50,2);
              perform mid_find_ic_surround_by_sapa_rec2(rec.link_id,50,3);
          end if;
          if rec.one_way_code in (2,3) then
              perform mid_find_ic_surround_by_sapa_rec2(rec.link_id, 50, rec.one_way_code);
          end if;
...
CREATE OR REPLACE FUNCTION mid_find_ic_surround_by_sapa_rec2( link bigint, max_num integer,dir integer)
	RETURNS integer
        LANGUAGE plpgsql
	AS $$
	DECLARE        	
      ...
      for i in 1..leng loop
          ...
          for rec in 
              ...
              where 	( ( a.s_node = enode or a.e_node = enode ) and slink = link
                    ...
                    and a.link_id <> temp_link 
                    and not (a.link_type in (3,7) or ( a.link_type in (1,2) and a.road_type in (0,1) ))
          loop

              if rec.link_id is not null then
              
              	      if rec.link_type <> 5 then
			return 1;
		      end if;
                  
                  if not (rec.link_id = any(link_searched))  then
                      temp_link_array := array_append(temp_link_array, rec.link_id);
                  end if;
              end if;
          end loop;
...
BEGIN
	...
	FOR rec IN 
		SELECT a.nodeid, a.inlinkid, a.outlinkid, a.inner_link_array, b.condtype, b.cond_id
	...
	LOOP
		...
		nCount := array_upper(rec.inner_link_array, 1);
		nIndex := 1;
		WHILE nIndex <= nCount LOOP
			IF nIndex = 1 THEN
	    		INSERT INTO regulation_item_tbl("regulation_id", "linkid", "nodeid", "seq_num")
	    			VALUES	(cur_regulation_id, rec.inner_link_array[nIndex], null, nIndex);
	    		INSERT INTO regulation_item_tbl("regulation_id", "linkid", "nodeid", "seq_num")
	    			VALUES	(cur_regulation_id, null, rec.nodeid, nIndex+1);
	    	ELSE
	    		INSERT INTO regulation_item_tbl("regulation_id", "linkid", "nodeid", "seq_num")
	    			VALUES	(cur_regulation_id, rec.inner_link_array[nIndex], null, nIndex+1);
	    	END IF;
			nIndex := nIndex + 1;
		END LOOP;
...
CREATE OR REPLACE FUNCTION mid_get_redundance_regulation_by_traffic_flow()
	RETURNS integer
	LANGUAGE plpgsql
	AS $$ 
DECLARE
...
BEGIN
	...
	LOOP
		...
		FOR link_loop in 2..rec.link_num LOOP
			...
			IF search_node = temp_s_node THEN
				search_node := temp_e_node;
			ELSE
...
BEGIN
  ...
  if path_count > 1 and (node = node_s or node = node_e) then
	...
	select link_id 
	from link_tbl as a
	where node in (a.s_node,a.e_node) and a.link_type in (1,2) and a.road_type in (0,1) and a.one_way_code not in (1,4) limit 1
	into slink;
		...
		if (node_s in (snode,enode) and node <> node_s )  or  (node_e in (snode,enode) and node <> node_e ) then   
			...
			for rec in 
				  ...
				  where (	  
					...
					and a.road_type in (0,1) and a.link_type in (1,2) and a.one_way_code not in (1,4)
			loop
				if rec.link_id is not null then
...
CREATE OR REPLACE FUNCTION mid_jct_change_to_ic_connect_to_highway_end_bak(node bigint,dir integer)
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	highway_count := 0;
	for rec in 
		select distinct link_id, link_type, road_type,one_way_code
		from link_tbl as a
		where node in (a.s_node, a.e_node) and a.link_type in (1,2) and a.road_type in (0,1) and a.one_way_code not in (1,4)
	...
	if highway_count = 0 then
		return 1;	
	end if;

	if highway_count > 1 then
		return 0;
	end if;
	...
	if highway_count = 1 then
		...
		for rec in
			...
			from link_tbl as a
			where (
                ( a.e_node = node and a.one_way_code = 2 
                  or
                  a.s_node = node and a.one_way_code = 3 
                )
                and dir = 1
                
             or 
             
               ( a.s_node = node and a.one_way_code = 2 
                 or
                 a.e_node = node and a.one_way_code = 3 
                )
               and dir = 2
       
             or 

			        node in	(a.s_node,a.e_node) and a.one_way_code = 1
				 
            )
				
				and 
				(not (a.link_type in (1,2) and a.road_type in (0,1) and a.one_way_code not in (1,4)))
				and a.link_type not in (3)
		loop
			if rec.link_id is not null then
        raise info 'rec.link_id = %',rec.link_id;
				general_count = general_count + 1;
...
CREATE OR REPLACE FUNCTION mid_jct_change_to_ic_connect_to_highway_end(node bigint,dir integer,highway_angle double precision)
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	for rec in 
		select distinct link_id, link_type, road_type,one_way_code
		from link_tbl as a
		where node in (a.s_node, a.e_node) and a.link_type in (1,2) and a.road_type in (0,1) 
		
	loop
		if rec.link_id is not null then
			highway_count = highway_count + 1;
			highway_link = rec.link_id;
	...
	if highway_count > 1 then
		return 0; --not endpoint or startpoint  jct
	end if;

	if highway_count = 1 then
    for rec in  
      select (case when (a.s_node = node and a.one_way_code = 2  or a.e_node = node and a.one_way_code = 3 ) then  1
                    when (a.e_node = node and a.one_way_code = 2  or a.s_node = node and a.one_way_code = 3 ) then  2
                    when (node in (a.s_node,a.e_node) and a.one_way_code = 1 ) then  3  end ) as highway_dir
      from link_tbl as a
      where link_id = highway_link
    loop
      highway_dir = rec.highway_dir;
		...
		for rec in
			...
			where (
             ...
             or 
				...
				and 
				(not (a.link_type in (1,2) and a.road_type in (0,1) ) )
				and a.link_type not in (3)
		loop
			if rec.link_id is not null then
        raise info 'rec.link_id = %',rec.link_id;
				general_count = general_count + 1;
				general_link_array = array_append(general_link_array,rec.link_id);
			end if;
...
CREATE OR REPLACE FUNCTION mid_jct_linktype_modify(node bigint, path bigint[], node_s bigint, node_e bigint, dir integer,angle double precision)
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	select mid_filter_the_large_angle_jct(path,angle) into flag3;
	...
	if flag3 = 1 then
		...
		if flag1 = 1 or flag2 = 1 then
			raise info 'highway_connect';
			insert into temp_jct_change_to_ic(links,link_type)
				values(path,5);
		else
      if dir = 1 then
            select mid_jct_change_to_ic_connect_to_highway_end(node,1,angle) into flag4;

            select mid_jct_change_to_ic_connect_to_highway_end(highway_node,2,angle) 
            from
            (
              select (case when node_s in (a.s_node,a.e_node) then node_s
                  when node_e in (a.s_node,a.e_node) then node_e end ) as highway_node
                  
              from link_tbl as a
              where (node_s in (a.s_node,a.e_node) or node_e in (a.s_node,a.e_node) ) and a.link_type in (1,2) and a.road_type in (0,1)
            ) as b
            into flag5;
			else
            select mid_jct_change_to_ic_connect_to_highway_end(node,2,angle) into flag4;

            select mid_jct_change_to_ic_connect_to_highway_end(highway_node,1,angle) 
            from
            (
              select (case when node_s in (a.s_node,a.e_node) then node_s
                  when node_e in (a.s_node,a.e_node) then node_e end ) as highway_node
...
CREATE OR REPLACE FUNCTION mid_regulation_exist_on_path_bak(path bigint[],dir integer)
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	if dir = 1 then
		i := 1;
		while i < array_length(path,1) loop
			for rec in
				select regulation_id
				from regulation_relation_tbl as a
				where a.inlinkid = path[i] and a.outlinkid = path[i+1] and a.condtype = 1
			loop
				if rec.regulation_id is not null then
					return 1;
				end if;
...
CREATE OR REPLACE FUNCTION mid_regulation_exist_on_path(path bigint[],dir integer)
	RETURNS smallint
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	for rec in
        ...
        from
        (
            select distinct regulation_id from regulation_relation_tbl
            where inlinkid = any(path) and condtype = 1			
        ) as a 
        left join regulation_item_tbl as b
        on a.regulation_id = b.regulation_id
        where b.linkid is not null
	...
	loop
		if dir = 1 then
        if rec.regulation_id is not null then 
              if rec.link_array <@ path then
                    i = 1;
                    while i <= array_length(path,1) loop
                        if path[i] = rec.link_array[1] then
                          s_index = i;
                          exit;
                        end if;
                        i = i + 1;
                    end loop;
                    length := array_length(rec.link_array,1);
                    j = 1;
                    flag = 0;
                    for i in s_index..array_length(path,1) loop
                        if j <= length then
                          if path[i] <> rec.link_array[j] then
                            flag = 1;
                            exit;
                          else 
                            j = j + 1;
                    ...
                    if flag = 0 and j = length + 1 then
                      raise info 'regulation exist on %',rec.link_array;
                      return 1;
                    end if;
		...
		if dir = 2 then
          if rec.regulation_id is not null then 
              if rec.link_array <@ path then
                    ...
                    i = 1;
                    while i <= array_length(path,1) loop
                        if path[i] = rec.link_array[length] then
                          s_index = i;
                          exit;
                        end if;
                        i = i + 1;
...
CREATE OR REPLACE FUNCTION mid_modify_jct_path_both_end_same(path1 bigint[],path2 bigint[])
	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	...
	for i in 1..length loop
		for rec in
			...
			on a.s_node in (b.s_node,b.e_node) or a.e_node in (b.s_node,b.e_node)
			where a.link_id = path1[i] 
			and a.link_id <> b.link_id
			and b.link_type not in (3,5,7)
			and not (b.link_type in (1,2) and b.road_type in (0,1))
		loop
			if rec.link_id is not null then
				flag1 = 1;
				exit;
	...
	if flag1 <> flag2 then
		if flag1 = 1 then
			insert into temp_jct_path_both_end_same(path,link_type)
				values(path1,5);
		else
			insert into temp_jct_path_both_end_same(path,link_type)
				values(path1,3);	
...
CREATE OR REPLACE FUNCTION mid_large_steering_angle_on_sapa_path_connect_highway(path bigint[], angle double precision)
  	RETURNS integer
	LANGUAGE plpgsql volatile
AS $$
DECLARE
...
BEGIN
	length :=  array_length(path,1);
	general_link = path[length - 1];
	highway_link = path[length];

	for rec in 
		select a.node_id
		from node_tbl as a
		join link_tbl as b
		on a.node_id in (b.s_node,b.e_node) and b.link_id = general_link
		join link_tbl as c
		on a.node_id in (c.s_node,c.e_node) and c.link_id = highway_link
	loop
		join_node := rec.node_id;

	end loop;
	
	select mid_calc_jct_steering_angle(general_link, highway_link, join_node) into steering_angle;
	raise info 'general_link = %, highway_link = %, join_node = %',general_link,highway_link,join_node;
	if steering_angle > angle then
		raise info 'the angle is ok, the angle = %', steering_angle;
		return 0;
	else
		raise info 'the angle has problem, the angle = %', steering_angle;
		return 1;
	end if;
...
CREATE OR REPLACE FUNCTION mid_delete_overdue_regulation_condition()
  RETURNS integer
  LANGUAGE plpgsql
AS $$
DECLARE
	...
	rec record;
BEGIN
	SELECT EXTRACT(year FROM now()) INTO n_year;
	SELECT EXTRACT(month FROM now()) INTO n_month;
	SELECT EXTRACT(day FROM now()) INTO n_day;
	--raise info '%', n_year;
	--raise info '%', n_month;
	--raise info '%', n_day;

	FOR rec IN
		SELECT cond_id, car_type, start_year, start_month, start_day, end_year, 
			end_month, end_day, start_hour, start_minute, end_hour, end_minute, 
			day_of_week, exclude_date
		FROM condition_regulation_tbl_bak_overdue
	LOOP
		IF rec.end_year != 0 THEN
			--raise info '%', rec;
			IF rec.end_year = n_year THEN
				IF rec.end_month = n_month THEN
					IF rec.end_day <= n_day THEN
						continue;
					END IF;
				ELSEIF rec.end_month < n_month THEN
					continue;
				END IF;
			ELSEIF rec.end_year < n_year THEN
				continue;
			END IF;
		END IF;
		INSERT INTO condition_regulation_tbl (cond_id, car_type, start_year, start_month, start_day, end_year, end_month, end_day, start_hour, start_minute, end_hour, end_minute, day_of_week, exclude_date) 
			VALUES (rec.cond_id, rec.car_type, rec.start_year, rec.start_month, rec.start_day, rec.end_year, rec.end_month, rec.end_day, rec.start_hour, rec.start_minute, rec.end_hour, rec.end_minute, rec.day_of_week, rec.exclude_date);
	END LOOP;
...
CREATE OR REPLACE FUNCTION mid_statistic_sapa_link_length(link bigint,max_num integer,suspect_link_array bigint[])
	RETURNS double precision
        LANGUAGE plpgsql
	AS $$
	DECLARE        	
...
BEGIN
	...
	while link_num < max_num loop
		...
		for i in 1..leng loop
			...
			select s_node, e_node
			from link_tbl as a
			where a.link_id = slink into  snode, enode;
			...
			for rec in 
				...
				from link_tbl as a
				where 
					( a.s_node in (snode, enode) or a.e_node in (snode,enode ) ) 
					and not (a.link_id = any(link_searched))
					and (
						a.link_id = any(suspect_link_array) 
						or 
						 a.link_type in (3,5,7) 
						 
						)
			loop
				if rec.link_id is not null then
					if rec.one_way_code <> 4 then
						temp_link_array := array_append(temp_link_array, rec.link_id);
					end if;
		...
		if leng is null then
			for rec in
				select link_id,link_type,length
				from link_tbl as a
				where a.link_id = any(link_arrayA)
			loop
				if rec.link_type = 7 then
					sapa_length = sapa_length + rec.length;
					raise info 'sapa_length = %',sapa_length;
					raise info 'sapa_link = %',rec.link_id;
...
CREATE OR REPLACE FUNCTION mid_statistic_sapa_link_length_bak(link bigint,max_num integer)
	RETURNS double precision
        LANGUAGE plpgsql
	AS $$
	DECLARE        	
...
CREATE OR REPLACE FUNCTION  mid_find_sapa_paths_start_ic(link bigint, dir integer,len integer)
	RETURNS smallint
        LANGUAGE plpgsql
	AS $$
	DECLARE 
...
BEGIN
    ...
    while i <= array_length(link_path_array,1) loop
        ...
        if array_length(link_array,1) > 1 then
            for rec in
                select (case when a.s_node in (b.s_node,b.e_node) then a.e_node
                              else a.s_node end ) as next_node
                from link_tbl as a
                join link_tbl as b
                on a.link_id = link_array[array_upper(link_array,1)] and b.link_id = link_array[array_upper(link_array,1) - 1]
            loop
                if rec.next_node is not null then
                    next_node = rec.next_node;
                end if;
            end loop;
         else
            select ( case when dir = 2 then a.e_node
                            when dir = 3 then a.s_node end ) as next_node
            from link_tbl as a
            where link_id = link_array[array_upper(link_array,1)] into next_node;
         end if;
        ...
        from link_tbl as a
        where a.link_id = slink  into  link_type1, road_type1;
        
        if link_type1 in (1,2) and road_type1 in (0,1) then
            if  sapa_count > 1 then
                  insert into temp_sapa_paths_start_ic(link_path) 
                        values((string_to_array(link_path_array[i],'|'))::bigint[]);
             else
                i = i + 1;
                continue;
            ...
            for rec in 
              ...
              from link_tbl as a
              where 	(
                  a.s_node = next_node and a.one_way_code in (1,2)
                  or
                  a.e_node = next_node and a.one_way_code in (1,3)
                )
                and ( a.link_type in (5,7) or (a.link_type in (1,2) and a.road_type in (0,1) and a.one_way_code <> 4))
                and a.link_id <> slink
            loop
              link_path_array := array_append(link_path_array,(link_path_array[i] || '|' || (rec.link_id)::varchar)::varchar);  
...
CREATE OR REPLACE FUNCTION mid_find_sapa_links_rev3( link bigint, dir integer, max_num integer, stat_sapa_max_num integer)
	RETURNS integer
        LANGUAGE plpgsql
	AS $$
	DECLARE        	
		...
		jct_ic_flag integer;
		hw_flag     integer;
		temp_link_array varchar[];
		flag_ic		integer;
		flag_jct_sapa_hw	integer;
		flag_end	integer;
		flag_rec_null	integer;
		all_sapa_length	double precision;
		all_general_length	double precision;
...
BEGIN
	...
	select (case when dir = 2 then a.e_node
		when dir = 3 then a.s_node end ) as link_node
	from link_tbl as a
	where a.link_id = link into link_node;
	
	link_arrayB := array_append(link_arrayB,(link::varchar || '|' || link_node::varchar)::varchar);
      while link_num < max_num loop
      leng := array_length(link_arrayB,1);
      if leng is null then
          leng := 0;
      ...
      for i in 1..leng loop
          slink :=  (split_part(link_arrayB[i],'|',1))::bigint;
          slink_node := (split_part(link_arrayB[i],'|',2))::bigint;
          ...
          raise info 'slink = %',link_arrayB[i];
          flag_rec_null := 1; 
          jct_ic_flag = 1;
          hw_flag = 0;
          temp_link_array = array[]::varchar[];
          for rec in 
              select link_id,link_type,road_type,one_way_code,
                      (case when a.s_node = slink_node then a.e_node
			   when a.e_node = slink_node then a.s_node end ) as join_node
              from link_tbl as a
	      where (	(a.s_node = slink_node or a.e_node = slink_node)
			and 
			a.link_id <> slink 
                        and 
                        not (a.link_id = any(link_searched))
		    )
          loop
              if rec.link_id is not null then
                  flag_rec_null = 0;
                  ...
                  if not (rec.link_id = any(link_searched)) and rec.link_type not in (3,5,7) and rec.one_way_code <> 4 then
                      ...
                      temp_link_array := array_append(temp_link_array, (rec.link_id::varchar || '|' || rec.join_node::varchar)::varchar);
                  end if;

              end if;
          end loop;
          if flag_rec_null = 1 then
              flag_end = 1;
          end if;
          ...
          raise info 'flag_ic = %',flag_ic;
          raise info 'flag_jct_sapa_hw = %',flag_jct_sapa_hw;
          raise info 'flag_end = %',flag_end;
      ...
      leng := array_length(link_arrayC,1);
      if leng is null then
      all_general_length = 0;
          if  flag_end = 1 then
              select mid_statistic_sapa_link_length(link ,stat_sapa_max_num,link_arrayA ) into all_sapa_length;
              for rec in
                  select distinct link_id, length,link_type
                  from link_tbl as a
                  where link_id = any(link_arrayA)
              loop
                  if rec.link_type <> 7 then
                    all_general_length = all_general_length + rec.length;
                  end if;

              end loop;
              raise info 'all_sapa_length = %',all_sapa_length;
              raise info 'all_general_length = %',all_general_length;
              general_sapa_percent = all_general_length / all_sapa_length;
              raise info 'general_sapa_percent = %',general_sapa_percent;
              insert into temp_sapa_link(link_array,road_end,general_sapa_percent)
                values(link_arrayA,1,general_sapa_percent);
                return 0;
             
          else
                insert into temp_sapa_link(link_array,road_end,general_sapa_percent)
                values(link_arrayA,0,0);
                return 1;
          end if;
      else
          link_arrayB = link_arrayC;
          link_arrayC = array[]::varchar[];
      end if;
...
CREATE OR REPLACE FUNCTION mid_find_sapa_links_rev3_bak( link bigint, dir integer, max_num integer, stat_sapa_max_num integer)
	RETURNS integer
        LANGUAGE plpgsql
	AS $$
	DECLARE        	
...
BEGIN
	...
	link_arrayB := array_append(link_arrayB,(link::varchar || '|' || '0')::varchar);
      while link_num < max_num loop
      leng := array_length(link_arrayB,1);
      if leng is null then
          leng := 0;
      ...
      for i in 1..leng loop
          ...
          for rec in 
              select link_id,link_type,road_type,one_way_code,
                      (case when a.s_node in (enode,snode) then a.s_node
                            else a.e_node end ) as join_node
              from link_tbl as a
              where 	( ( a.s_node = enode or a.e_node = enode ) and slink = link 
                        or
                        ( a.s_node in (snode, enode) and a.s_node <> slink_node or a.e_node in (snode,enode ) and a.e_node <> slink_node ) and slink <> link 
                        )
                      and a.link_id <> slink 
                      and not (a.link_id = any(link_searched))
          loop
...
CREATE OR REPLACE FUNCTION mid_find_sapa_paths_start_ic_exec(max_num integer)
	RETURNS integer
        LANGUAGE plpgsql
	AS $$
	DECLARE  
...
BEGIN
	for rec in
		select distinct a.link_id,a.one_way_code
		from 
		(
			select *
			from link_tbl 
	...
	loop
		if rec.one_way_code = 1 then
			perform mid_find_sapa_paths_start_ic(rec.link_id::bigint, 2, max_num);
			perform mid_find_sapa_paths_start_ic(rec.link_id::bigint, 3, max_num);
			
		else
			perform mid_find_sapa_paths_start_ic(rec.link_id::bigint, rec.one_way_code, max_num);
