...
# HwyFacilityName
# =============================================================================
class HwyFacilityName(component.component_base.comp_base):
    '''
    ...
    def __init__(self):
        ...
        Constructor
        '''
        component.component_base.comp_base.__init__(self, 'HwyFacilityName')
                ...
                for facil, link_lid, node_lid, path_type in path_infos:
                    ...
                    if(path_type == 'IC' and
                       ...
                       self._is_on_no_toll_main_link(node_lid[-1])):
                        self.log.info('Last Node in HWY no_toll main link。'
                                      'node_id=%s,LastNode=%s,name=%s'
    ...
    def _in_sapa_shortest_path(self, sapa_facil, link_id):
        ...
        if link_id in shortest_path:
            return True
        link_lid, node_lid = self._get_shortest_path_through_sapa_pos(path_infos)
        if link_id in link_lid:
            hwy_data = HwyDataMng.instance()
            link_type = hwy_data.get_link_type(link_id)
            sapa_pos = self.get_sapa_pos(node_lid)
            if(link_type != LINK_TYPE_SAPA and
               link_lid.index(link_id) >= sapa_pos):  # link在SAPA所在点前(车流方向)
                pass
            else:  # link在SAPA所在点后方(车流方向)
                return True
        return False
    ...
    def _get_shortest_path_through_sapa_pos(self, path_infos):
        ...
        shortest_len = 0
        for facil, link_lid, node_lid, path_type in path_infos:
            if path_type == 'SAPA' and self._is_through_sapa_pos(node_lid):
                if len(link_lid) < shortest_len or shortest_len == 0:
                    shortest_path = link_lid
                    shortest_node_list = node_lid
                    shortest_len = len(link_lid)
        return shortest_path, shortest_node_list

    def _get_shortest_path_no_through_sapa_pos(self, path_infos):
        '''不经过SAPA设施所在点的最短路径. 返回的是[link序】'''
        shortest_path = []
        shortest_len = 0
        for facil, link_lid, node_lid, path_type in path_infos:
            if path_type == 'SAPA' and not self._is_through_sapa_pos(node_lid):
                if len(link_lid) < shortest_len or shortest_len == 0:
                    shortest_path = link_lid
                    shortest_len = len(link_lid)
        ...
        for bwd in bwd_list:
            ...
            if path_type in ('SAPA'):
                ...
                if self._is_smart_ic(bwd_facil):  # 后方设施是SmartIC
                    ...
                    # IC, JCT的路径
                    if temp_path_type != 'SAPA' and link_type == LINK_TYPE_SAPA:
