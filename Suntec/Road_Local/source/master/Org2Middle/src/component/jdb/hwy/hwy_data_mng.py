...
from component.default.link_graph import load_link_by_box
from component.default.link_graph import get_node_expand_box
UNITS_TO_EXPAND_NO_TOLL = 0.01


class HwyDataMng(component.component_base.comp_base):
    '''
    classdocs
    '''
    __instance = None
    ...
    def __init__(self):
        ...
        Constructor
        '''
        component.component_base.comp_base.__init__(self, 'HwyDataMng')
        ...
        self.__boundary_node_dict = {}  # Hwy Tile边界点
        self.__link_mapping = {}
    ...
    def load_hwy_data(self):
        ...
        # self.__load_facility() # load_hwy_main_links里已经执行过
        # 无料区间
    ...
    def __load_facility(self):
        ...
        for fac_info in data:
            ...
            dummyfacils = fac_info[8]
            ficilties = []
            for facilcls, nodeid, inout, facil_name, name_yomi, dummyfacil in \
                    zip(facilcls_list, node_lid, inout_list,
                        facil_names, name_yomis, dummyfacils):
                fac = HwyFacilInfo(roadcode, roadpoint, facilcls,
                                   updown, nodeid, inout,
                                   facil_name, name_yomi,
                                   dummyfacil=dummyfacil)
        ...
        for ic in data:
            ...
            for nodeid, inout in zip(nodeids, inouts):
                ...
                facil = HwyFacilInfo(roadcode, roadpoint, facilcls,
                                     ...
                                     tollclass, dummytoll, dummyfacil)
...
#             facil.inout == INOUT_TYPE_IN):
    ...
    def get_link_mapping_info(self, link_id):
        if not self.__link_mapping:
            self._load_link_mapping_info()
        info = self.__link_mapping.get(link_id)
        if info:
            return info
        return None

    def get_mapping_bwd_ic(self, link_id):
        mapping_info = self.get_link_mapping_info(link_id)
        if mapping_info:
            return mapping_info[0]  # bwd ic nos
        return set()

    def get_mapping_fwd_ic(self, link_id):
        mapping_info = self.get_link_mapping_info(link_id)
        if mapping_info:
            return mapping_info[1]  # fwd ic nos
        return set()

    def get_mapping_road_info(self, link_id):
        # (road_kind, road_no)
        mapping_info = self.get_link_mapping_info(link_id)
        if mapping_info:
            road_kind, road_no = mapping_info[2:4]
            return road_kind, road_no
        return None, None

    def _load_link_mapping_info(self):
        sqlcmd = """
        SELECT link_id, road_kind, road_no,
               array_agg(backward_ic_no),
               array_agg(forward_ic_no)
          FROM highway_mapping
          GROUP BY link_id, road_kind, road_no;
        """
        for info in self.pg.get_batch_data2(sqlcmd):
            link_id, road_kind, road_no = info[0:3]
            bwd_ic_nos, forward_ic_no = info[3:5]
            bwd_ic_nos = set(bwd_ic_nos)
            forward_ic_no = set(forward_ic_no)
            if(bwd_ic_nos == set([INVALID_IC_NO]) or
               bwd_ic_nos == set([None])):
                bwd_ic_nos = set()
            if(forward_ic_no == set([INVALID_IC_NO]) or
               forward_ic_no == set([None])):
                forward_ic_no = set()
            self.__link_mapping[link_id] = (bwd_ic_nos, forward_ic_no,
                                            road_kind, road_no)
...
class HwyFacilInfo(object):
    def __init__(self, roadcode, roadpoint, facilcls,
                 ...
                 tollclass=0, dummytoll=0, dummyfacil=0):
        '''
        Constructor
        '''
