...
class comp_node_rdf(component.component_base.comp_base):
    '''
    classdocs
    '''
    ...
    def _Do(self):
        ...
        sqlcmd = """
            ...
            from rdf_condition as a
            left join rdf_nav_strand as b
            on a.nav_strand_id = b.nav_strand_id 
            where a.condition_type = 9
            and b.node_id is not null
        """
        self.pg.execute2(sqlcmd)
        self.pg.commit2()
        self.CreateIndex2('temp_node_bifurcation_node_id_idx')  
        ...
        sqlcmd = """
                ...
                select 
                    ...
                    case when e.node_id is null then 0 else 1 end as stopsign_flag,
